...
log_data_addr::log_data_addr(const VFID *vfid_arg, PAGE_PTR pgptr_arg, PGLENGTH offset_arg)
    : vfid(vfid_arg), pgptr(pgptr_arg), offset(offset_arg)
{
}

log_append_info::log_append_info()
    : vdes(NULL_VOLDES), nxio_lsa(NULL_LSA), prev_lsa(NULL_LSA), log_pgptr(NULL), appending_page_tde_encrypted(false)
{
}

log_append_info::log_append_info(const log_append_info &other)
    : vdes(other.vdes), nxio_lsa{other.nxio_lsa.load()}, prev_lsa(other.prev_lsa), log_pgptr(other.log_pgptr), appending_page_tde_encrypted(other.appending_page_tde_encrypted)
{
}
...
log_prior_lsa_info::log_prior_lsa_info()
    : prior_lsa(NULL_LSA), prev_lsa(NULL_LSA), prior_list_header(NULL), prior_list_tail(NULL), list_size(0), prior_flush_list_header(NULL), prior_lsa_mutex()
{
}
                                          ...
                                          int num_rcrumbs, const LOG_CRUMB *rcrumbs)
{
  ...
  if (can_zip == true && (ulength >= log_Zip_min_size_to_compress || rlength >= log_Zip_min_size_to_compress))
  {
    ...
    if (data_ptr != NULL)
    {
      ...
      assert(CAST_BUFLEN(tmp_ptr - data_ptr) == total_length || ulength < log_Zip_min_size_to_compress || rlength < log_Zip_min_size_to_compress);
  ...
  case LOG_DIFF_UNDOREDO_DATA:
    undoredo_p = ((node->log_header.type == LOG_UNDOREDO_DATA || node->log_header.type == LOG_DIFF_UNDOREDO_DATA)
                      ? (LOG_REC_UNDOREDO *)node->data_header
  ...
  if (vacuum_info_p != NULL)
  {
    ...
    assert(node->log_header.type == LOG_MVCC_UNDO_DATA || node->log_header.type == LOG_MVCC_UNDOREDO_DATA || node->log_header.type == LOG_MVCC_DIFF_UNDOREDO_DATA);
    assert(LOG_IS_MVCC_OPERATION(rcvindex));
...
prior_lsa_next_record_internal(THREAD_ENTRY *thread_p, LOG_PRIOR_NODE *node, LOG_TDES *tdes, int with_lock)
{
  ...
  if (LOG_ISRESTARTED() && log_Gl.hdr.does_block_need_vacuum)
  {
    ...
    {
      assert(vacuum_get_log_blockid(log_Gl.hdr.mvcc_op_log_lsa.pageid) <= (vacuum_get_log_blockid(start_lsa.pageid) - 1));
  ...
  if (node->log_header.type == LOG_MVCC_UNDO_DATA || node->log_header.type == LOG_MVCC_UNDOREDO_DATA || node->log_header.type == LOG_MVCC_DIFF_UNDOREDO_DATA || (node->log_header.type == LOG_SYSOP_END && ((LOG_REC_SYSOP_END *)node->data_header)->type == LOG_SYSOP_END_LOGICAL_MVCC_UNDO))
  {
    ...
    else
    {
      ...
      assert(node->log_header.type == LOG_MVCC_UNDOREDO_DATA || node->log_header.type == LOG_MVCC_DIFF_UNDOREDO_DATA);
  ...
  else if (node->log_header.type == LOG_SYSOP_END)
  {
    ...
    if (!LSA_ISNULL(&tdes->rcv.atomic_sysop_start_lsa) && LSA_LT(&sysop_end->lastparent_lsa, &tdes->rcv.atomic_sysop_start_lsa))
    {
      /* atomic system operation finished. */
      LSA_SET_NULL(&tdes->rcv.atomic_sysop_start_lsa);
    }
    if (!LSA_ISNULL(&tdes->rcv.sysop_start_postpone_lsa) && LSA_LT(&sysop_end->lastparent_lsa, &tdes->rcv.sysop_start_postpone_lsa))
    {
      /* atomic system operation finished. */
