...
#define LOG_ISCHECKPOINT_TIME() \
  (log_Gl.rcv_phase == LOG_RESTARTED && log_Gl.run_nxchkpt_atpageid != NULL_PAGEID && log_Gl.hdr.append_lsa.pageid >= log_Gl.run_nxchkpt_atpageid)
...
#define LOG_NEED_TO_SET_LSA(RCVI, PGPTR) \
  (((RCVI) != RVBT_MVCC_INCREMENTS_UPD) && ((RCVI) != RVBT_LOG_GLOBAL_UNIQUE_STATS_COMMIT) && ((RCVI) != RVBT_REMOVE_UNIQUE_STATS) && ((RCVI) != RVLOC_CLASSNAME_DUMMY) && ((RCVI) != RVDK_LINK_PERM_VOLEXT || !pgbuf_is_lsa_temporary(PGPTR)))

#define CDC_IS_IGNORE_LOGINFO_ERROR(ERROR) \
  (((ERROR) == ER_CDC_IGNORE_LOG_INFO) || ((ERROR) == ER_CDC_IGNORE_LOG_INFO_INTERNAL) || ((ERROR) == ER_CDC_IGNORE_TRANSACTION))
 ...
 *              is initialized.
 */
int log_create(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath, const char *prefix_logname,
               DKNPAGES npages)
{
 ...
 *              database disk.
 */
void log_initialize(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath, const char *prefix_logname,
 ...
 */
void log_restart_emergency(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath,
 ...
 *
 */
void log_append_undoredo_data(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr, int undo_length,
...
void log_append_undoredo_data2(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VFID *vfid, PAGE_PTR pgptr,
 ...
 *              record.
 */
void log_append_undo_data(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr, int length,
                          const void *data)
{
  LOG_CRUMB undo_crumb;
...
void log_append_undo_data2(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VFID *vfid, PAGE_PTR pgptr,
 ...
 *              ignored.
 */
void log_append_redo_data(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr, int length,
                          const void *data)
{
  LOG_CRUMB redo_crumb;
...
void log_append_redo_data2(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VFID *vfid, PAGE_PTR pgptr,
                           PGLENGTH offset, int length, const void *data)
{
  LOG_DATA_ADDR addr;
  LOG_CRUMB redo_crumb;
 ...
 *
 */
void log_append_undoredo_crumbs(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr, int num_undo_crumbs,
                                int num_redo_crumbs, const LOG_CRUMB *undo_crumbs, const LOG_CRUMB *redo_crumbs)
{
  ...
  {
    if (rcvindex == RVHF_UPDATE || rcvindex == RVOVF_CHANGE_LINK || rcvindex == RVHF_UPDATE_NOTIFY_VACUUM || rcvindex == RVHF_INSERT_NEWHOME)
    {
 ...
 *
 */
void log_append_undo_crumbs(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr, int num_crumbs,
 ...
 */
void log_append_redo_crumbs(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr, int num_crumbs,
                            const LOG_CRUMB *crumbs)
{
  LOG_TDES *tdes; /* Transaction descriptor */
  int tran_index;
 ...
 *
 */
void log_append_undoredo_recdes(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr,
...
void log_append_undoredo_recdes2(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VFID *vfid, PAGE_PTR pgptr,
...
void log_append_undo_recdes2(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VFID *vfid, PAGE_PTR pgptr,
...
void log_append_redo_recdes2(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VFID *vfid, PAGE_PTR pgptr,
                             PGLENGTH offset, const RECDES *recdes)
{
  LOG_CRUMB crumbs[2];
  LOG_DATA_ADDR addr;
...
void log_append_postpone(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr, int length, const void *data)
{
  ...
  if (LSA_ISNULL(&tdes->tail_lsa) || (log_is_in_crash_recovery() && (crash_lsa = log_get_crash_point_lsa()) != NULL && LSA_LE(&tdes->tail_lsa, crash_lsa)))
  {
 ...
 */
void log_append_run_postpone(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_DATA_ADDR *addr, const VPID *rcv_vpid,
                             int length, const void *data, const LOG_LSA *ref_lsa)
{
  ...
  if (tdes->state != TRAN_UNACTIVE_WILL_COMMIT && tdes->state != TRAN_UNACTIVE_COMMITTED_WITH_POSTPONE && tdes->state != TRAN_UNACTIVE_TOPOPE_COMMITTED_WITH_POSTPONE)
  {
 ...
 */
void log_append_compensate(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VPID *vpid, PGLENGTH offset,
 ...
 */
void log_append_compensate_with_undo_nxlsa(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VPID *vpid,
...
void log_sysop_start(THREAD_ENTRY *thread_p)
{
  ...
  // thread 에서 tran_index 를 찾고, tdes 를 찾는다.
  tran_index = LOG_FIND_THREAD_TRAN_INDEX(thread_p);
  tdes = LOG_FIND_TDES(tran_index);
  if (tdes == NULL)
  {
  ...
  // reenternce lock(재 진입 가능한) 뮤택스 락을 건다.
  tdes->lock_topop();

  /* Can current tdes.topops stack handle another system operation? */
  // 현재 tdes.topops stack 이 또 다른 시스템 작업을 처리할 수 있는지 확인한다.
 ...
 */
void log_sysop_commit_internal(THREAD_ENTRY *thread_p, LOG_REC_SYSOP_END *log_record, int data_size, const char *data,
                               bool is_rv_finish_postpone)
{
  ...
  if ((LSA_ISNULL(&tdes->tail_lsa) || LSA_LE(&tdes->tail_lsa, LOG_TDES_LAST_SYSOP_PARENT_LSA(tdes))) && (log_record->type == LOG_SYSOP_END_COMMIT || log_No_logging))
  {
    /* No change. */
  ...
  else
  {
    ...
    if (log_record->type == LOG_SYSOP_END_LOGICAL_RUN_POSTPONE)
    {
      ...
      assert(tdes->state == TRAN_UNACTIVE_COMMITTED_WITH_POSTPONE || tdes->state == TRAN_UNACTIVE_TOPOPE_COMMITTED_WITH_POSTPONE);
    ...
    else if (log_record->type == LOG_SYSOP_END_LOGICAL_COMPENSATE)
    {
      ...
      assert(tdes->state == TRAN_UNACTIVE_ABORTED || tdes->state == TRAN_UNACTIVE_UNILATERALLY_ABORTED || (is_rv_finish_postpone && (tdes->state == TRAN_UNACTIVE_TOPOPE_COMMITTED_WITH_POSTPONE || tdes->state == TRAN_UNACTIVE_COMMITTED_WITH_POSTPONE)));
    }
    ...
    else
    {
      ...
      assert(tdes->state != TRAN_UNACTIVE_COMMITTED_WITH_POSTPONE && (tdes->state != TRAN_UNACTIVE_TOPOPE_COMMITTED_WITH_POSTPONE || is_rv_finish_postpone));
    }

    if (!LOG_CHECK_LOG_APPLIER(thread_p) && tdes->is_active_worker_transaction() && log_does_allow_replication() == true)
    {
 ...
 *       was not necessary.
 */
void log_sysop_end_logical_undo(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, const VFID *vfid, int undo_size,
 ...
 * data (in)       : undo data
 */
void log_sysop_end_recovery_postpone(THREAD_ENTRY *thread_p, LOG_REC_SYSOP_END *log_record, int data_size,
                                     const char *data)
{
...
int log_append_supplemental_lsa(THREAD_ENTRY *thread_p, SUPPLEMENT_REC_TYPE rec_type, OID *classoid, LOG_LSA *undo_lsa,
                                LOG_LSA *redo_lsa)
{
  int size;
...
int log_append_supplemental_serial(THREAD_ENTRY *thread_p, const char *serial_name, int cached_num, OID *classoid,
...
log_commit(THREAD_ENTRY *thread_p, int tran_index, bool retain_lock)
{
  ...
  // 상위 루틴에서도 LOG_FIND_THREAD_TRAN_INDEX를 호출해서 tran id를 조회하는데, 
  // NULL_TRAN_INDEX일경우 동일 함수를 한번 더 호출한다.
  // 이 사이에 달라질 가능성이 있는 루틴인가??
  ...
  // log_2pc_commit(prepare & ) 또는 log_commit_local & log_complete을 호출한다.
  // 
 ...
 *              This function is used for debugging purposes.
 */
void xlog_dump(THREAD_ENTRY *thread_p, FILE *out_fp, int isforward, LOG_PAGEID start_logpageid, DKNPAGES dump_npages,
  ...
  while (!LSA_ISNULL(&lsa) && dump_npages-- > 0)
  {
    ...
    while (lsa.pageid == log_lsa.pageid)
    {
...

      {
        ...
        if (log_startof_nxrec(thread_p, &next_lsa, false) == NULL || (!LSA_EQ(&next_lsa, &log_rec->forw_lsa) && !LSA_ISNULL(&log_rec->forw_lsa)))
        {
          fprintf(out_fp, "\n\n>>>>>****\n");
...
log_undo_rec_restartable(THREAD_ENTRY *thread_p, LOG_RCVINDEX rcvindex, LOG_RCV *rcv)
{
  ...
  } while (++num_retries <= LOG_REC_UNDO_MAX_ATTEMPTS && (error_code == ER_LK_PAGE_TIMEOUT || error_code == ER_LK_UNILATERALLY_ABORTED));
 ...
 */
int log_get_next_nested_top(THREAD_ENTRY *thread_p, LOG_TDES *tdes, LOG_LSA *start_postpone_lsa,
...
void log_do_postpone(THREAD_ENTRY *thread_p, LOG_TDES *tdes, LOG_LSA *start_postpone_lsa)
{
  ...
  assert(tdes->state == TRAN_UNACTIVE_TOPOPE_COMMITTED_WITH_POSTPONE || tdes->state == TRAN_UNACTIVE_WILL_COMMIT || tdes->state == TRAN_UNACTIVE_COMMITTED_WITH_POSTPONE);
  ...
  while (!LSA_ISNULL(&next_start_seek_lsa))
  {
    ...
    while (!LSA_ISNULL(&forward_lsa) && !isdone)
    {
      ...
      while (forward_lsa.pageid == log_lsa.pageid)
      {
        ...
        if (forward_lsa.offset == NULL_OFFSET && forward_lsa.pageid != NULL_PAGEID && forward_lsa.pageid < log_lsa.pageid)
        {
 ...
 *                      Database Backup = db_backup
 */
bool log_is_active_log_sane(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath,
                            const char *prefix_logname)
{
  LOG_HEADER hdr;
  REL_COMPATIBILITY compat;
 ...
 */
int log_recreate(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath, const char *prefix_logname,
 ...
 */
int log_get_charset_from_header_page(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath,
                                     const char *prefix_logname)
{
...
log_get_undo_record(THREAD_ENTRY *thread_p, LOG_PAGE *log_page_p, LOG_LSA process_lsa, RECDES *recdes)
{
  ...
  {
    assert_release(log_rec_header->type == LOG_MVCC_UNDO_DATA || log_rec_header->type == LOG_MVCC_UNDOREDO_DATA || log_rec_header->type == LOG_MVCC_DIFF_UNDOREDO_DATA || log_rec_header->type == LOG_UNDO_DATA || log_rec_header->type == LOG_UNDOREDO_DATA || log_rec_header->type == LOG_MVCC_DIFF_UNDOREDO_DATA || log_rec_header->type == LOG_SUPPLEMENTAL_INFO);
 ...
 */
int log_read_sysop_start_postpone(THREAD_ENTRY *thread_p, LOG_LSA *log_lsa, LOG_PAGE *log_page, bool with_undo_data,
                                  ...
                                  char **undo_buffer, int *undo_size, char **undo_data)
{
  ...
  *sysop_start_postpone = *(LOG_REC_SYSOP_START_POSTPONE *)(log_page->area + log_lsa->offset);
  if (!with_undo_data || (sysop_start_postpone->sysop_end.type != LOG_SYSOP_END_LOGICAL_UNDO && sysop_start_postpone->sysop_end.type != LOG_SYSOP_END_LOGICAL_MVCC_UNDO))
  {
    /* no undo */
    return NO_ERROR;
  }
...
public:
  log_remove_log_archive_daemon_task()
      : m_is_timed_wait(true), m_period(0), m_param_period(0), m_log_deleted_time()
  {
    // initialize period
    compute_period();
  }
...
cdc_loginfo_producer_execute(cubthread::entry &thread_ref)
{
  ...
  while (cdc_Gl.producer.request != CDC_REQUEST_PRODUCER_TO_BE_DEAD)
  {
    ...
    if (LSA_GE(&cdc_Gl.producer.next_extraction_lsa, &nxio_lsa))
    {
      ...
      cdc_log("cdc_loginfo_producer_execute : next_extraction_lsa (%lld | %d)  is greater or equal than nxio_lsa (%lld | %d)",
    ...
    {
      cdc_log("cdc_loginfo_producer_execute : cdc_log_extract() error(%d) is returned at extracting log from lsa (%lld | %d)",
    ...
    if (error == ER_CDC_LOGINFO_ENTRY_GENERATED)
    {
      ...
      if (tmp == NULL)
      {
        cdc_log("cdc_loginfo_producer_execute : failed to allocate memory for log info entry of LOG_LSA (%lld | %d)",
...
int cdc_get_recdes(THREAD_ENTRY *thread_p, LOG_LSA *undo_lsa, RECDES *undo_recdes, LOG_LSA *redo_lsa,
                   RECDES *redo_recdes, bool is_flashback)
{
  ...
  if (redo_lsa != NULL)
  {
    ...
    {
      if (cdc_Gl.producer.temp_logbuf[tmpbuf_index].log_page_p->hdr.logical_pageid == redo_lsa->pageid && !is_flashback)
      {
        memcpy(log_page_p, cdc_Gl.producer.temp_logbuf[tmpbuf_index].log_page_p, IO_MAX_PAGE_SIZE);
      }
      else
                        ...
                        LOG_RCVINDEX rcvindex, bool is_redo)
{
  ...
  if (((current_log_record->type == LOG_UNDO_DATA) && (rcvindex == RVOVF_PAGE_UPDATE) && is_redo) || ((current_log_record->type == LOG_REDO_DATA) && (rcvindex == RVOVF_PAGE_UPDATE) && !is_redo) || rcvindex == RVOVF_CHANGE_LINK || rcvindex == RVOVF_NEWPAGE_LINK)
  {
  ...
  while (!LSA_ISNULL(&current_lsa))
  {
    ...
    else if ((LOG_IS_REDO_RECORD_TYPE(current_log_record->type) && is_redo) || (LOG_IS_UNDO_RECORD_TYPE(current_log_record->type) && !is_redo))
    {
      ovf_list_data = (OVF_PAGE_LIST *)malloc(DB_SIZEOF(OVF_PAGE_LIST));
      if (ovf_list_data == NULL)
      {
...
int cdc_make_dml_loginfo(THREAD_ENTRY *thread_p, int trid, char *user, CDC_DML_TYPE dml_type,
...
cdc_make_ddl_loginfo(char *supplement_data, int trid, const char *user, CDC_LOGINFO_ENTRY *ddl_entry)
{
  ...
  cdc_log("cdc_make_ddl_loginfo : started with trid:%d, transaction user:%s, class oid:(%d|%d|%d), ddl type:%d, object type:%d",
  ...
  loginfo_length = (OR_INT_SIZE + OR_INT_SIZE + or_packed_string_length(user, NULL) + OR_INT_SIZE + OR_INT_SIZE + OR_INT_SIZE + OR_BIGINT_SIZE + OR_BIGINT_SIZE + OR_INT_SIZE + statement_length);
...
cdc_find_user(THREAD_ENTRY *thread_p, LOG_LSA process_lsa, int trid, char **user)
{
  ...
  cdc_log("cdc_find_user : failed to find transaction user for TRANID (%d) because the supplemental log for trasaction user is not logged",
          trid);
  return ER_FAILED;
}
...
void cdc_reinitialize_queue(LOG_LSA *start_lsa)
{
  ...
  {
    cdc_log("cdc_reinitialize_queue : reconstruct existing log info queue to remove the log infos before the LOG_LSA (%lld | %d)",
...
int cdc_make_loginfo(THREAD_ENTRY *thread_p, LOG_LSA *start_lsa)
{
  ...
  cdc_log("cdc_make_loginfo : consume the log info entry in the queue and send to the requester.\nnumber of loginfos:(%d), total length of loginfos:(%d), next LOG_LSA to consume:(%lld | %d).",
