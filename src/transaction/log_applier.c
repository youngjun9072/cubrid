...
static inline bool la_is_valid_log_buffer(const LA_CACHE_BUFFER *log_buffer, LOG_PAGEID pageid)
{
  return log_buffer->pageid == pageid && 
    log_buffer->logpage.hdr.logical_pageid == pageid && 
    log_buffer->logpage.hdr.offset > NULL_OFFSET;
}

/*
 * la_does_page_exist() - check whether the page exists in the archive log or the active log.
 *   return: LA_PAGE_DOESNOT_EXIST if not found; otherwise, archive/active log.
 *   pageid(in): the target page id
 */
static int la_does_page_exist(LOG_PAGEID pageid)
{
  LA_CACHE_BUFFER *log_buffer = la_get_page_buffer(pageid);
  
  if(log_buffer == NULL)
    goto exit;

  if (la_is_valid_log_buffer(log_buffer, pageid))
  {
    /* The page is in the cache */
    la_release_page_buffer(pageid);
    
    return log_buffer->in_archive ? LA_PAGE_EXST_IN_ARCHIVE_LOG : LA_PAGE_EXST_IN_ACTIVE_LOG;
  }
  
  la_invalidate_page_buffer(log_buffer);

exit: 
  return LA_PAGE_DOESNOT_EXIST;
}
...
static inline bool la_check_skip_archive_page(int pageid)
{
  return la_does_page_exist(final->pageid + 1) && 
        la_does_page_exist(final->pageid) == LA_PAGE_EXST_IN_ARCHIVE_LOG
}
...
la_log_record_process(LOG_RECORD_HEADER *lrec, LOG_LSA *final, LOG_PAGE *pg_ptr)
{
  ...
  {
  case LOG_END_OF_LOG:
    if (la_check_skip_archive_page(final->pageid)){
  ...
  case LOG_DUMMY_HA_SERVER_STATE:
    LOG_REC_HA_SERVER_STATE *ha_server_state = la_get_ha_server_state(pg_ptr, final);
