...
#define LA_MOVE_INSIDE_RECORD(rec, dest_offset, src_offset)                                                    \
  ...
  {                                                                                                            \
    ...
    assert(((rec)->area_size <= 0) || (((rec)->length + ((dest_offset) - (src_offset))) <= (rec)->area_size)); \
...
la_log_phypageid(LOG_PAGEID logical_pageid)
{
  ...
  {
    // tmp_pageid 는 계산 후에 물레 페이지 아이디 변수에 넣어줌
    LOG_PAGEID tmp_pageid;
    // fpageid 는 active log에서 물리위치 1에 있는 논리적 페이지 id
    tmp_pageid = logical_pageid - la_Info.act_log.log_hdr->fpageid;
    // ex) logical_pageid 가 1100이고, act_log에서 시작 페이지 id가 1000이면
    // tmp_pageid 는 100

    // tmp_pageid 가 la_Info.act_log.log_hdr->npages 이상이거나
    //  음수일 경우 다시 순환 위치 조정해주고,
    //  물리 페이지 아이디는 1부터 시작하므로 1증가
 ...
 *     reads a predefined size of page from the disk
 */
// vname 은 로드 파일의 경로 및 이름인데, 에러 상황에서만 딱한번 호출된다.
// fd를 이용해 역참조하거나, la_Info.act_log 만 전달해도 무방해보인다.
static int
...
la_find_archive_num(int *arv_log_num, LOG_PAGEID pageid)
{
  ...
  do
  {
    if (la_Info.arv_log.log_vdes != NULL_VOLDES && la_Info.arv_log.log_hdr != NULL && guess_num == la_Info.arv_log.log_hdr->arv_num)
    {
      // la_Info.arv_log.log_hdr는 업데이트 되는 내용이 아닌것같은데 반복해서 변수에 넣을 필요가 있는건가???
...
la_log_fetch_from_archive(LOG_PAGEID pageid, char *data)
{
  ...
  {
    arv_log_num = -1;
    // pageid 를 갖고있는 archive log 번호를 찾는다.
...
log_reopen:
  ...
  if (la_Info.arv_log.log_hdr == NULL || strncmp(la_Info.arv_log.log_hdr->magic, CUBRID_MAGIC_LOG_ARCHIVE, CUBRID_MAGIC_MAX_LENGTH) != 0 || la_Info.arv_log.log_hdr->arv_num != la_Info.arv_log.arv_num)
  {
    error =
...
/*
로그 페이지를 active(또는 archive)로부터 읽는다.
active 에서는 이미 파일 디스크립터를 확보하고 있는 반면,
archive는 validation 및 파일을 오픈하는 루틴을 포함한다.
공통적으로는 la_log_io_read_with_max_retries를 통해 파일의 내용을 읽는다.

읽혀진 파일은 캐시 버퍼의 logpage에 저장된다.
...
la_log_fetch(LOG_PAGEID pageid, LA_CACHE_BUFFER *cache_buffer)
{
  ...
  do
  {
    ...
    if (LA_LOG_IS_IN_ARCHIVE(pageid))
    {
      ...
      // arvhive 파일을 열어 헤더와 로그 데이터를 읽고, 데이터는 cache_buffer->logpage에 저장한다.
    ...
    else
    {
      ...
      // 파일 내용을 읽어 cache_buffer->logpage(포인터 주소)에 저장한다.
      // logpage는 header와 data를 포함한다.
      error =
    ...
    {
      // logpage 이런 자료구조들 한번 더 살펴보고 어떻게 그려지는지 알아둬야함
      break;
    }
 ...
 *           size.
 */
// slb = single log buffer 가 더 가능성 있을 듯
static int
la_expand_cache_log_buffer(LA_CACHE_PB *cache_pb, int slb_cnt, int slb_size)
{
  ...
  // 현재 캐시 페이지 버퍼의 개수에 새로 할당할 slb 의 개수를 더한만큼의 slb_log_buffer를 재할당한다.
  ...
  // 위에서 재할당한 slb_log_buffer를 캐시 페이지 버퍼에 할당한다.
  // ?????? area는 재할당 안해도 되는건가??? 그대로 대입이면 이전 데이터는???
...
la_cache_buffer_replace(LA_CACHE_PB *cache_pb, LOG_PAGEID pageid, int io_pagesize, int buffer_size)
{
  ...
  while (found < 0)
  {
    ...
    // 캐시 버퍼를 순회하며 fix_count가 0인 캐시 버퍼를 찾는다.
    // ???? recentrly_free에 대한 처리 는 무엇인지?
    ...
    // 캐시 버퍼를 찾을 경우 해시 테이블에서 데이터를 제거하고,
    // 캐시 버퍼를 다시 패치해 덮어 쓴다.
    // 실패할 경우 캐시 버퍼의 페이지 아이디를 0으로 초기화하고 종료한다.
    // 실제 데이터는 캐시 버퍼의 logpage 에 저장된다.
    if (found >= 0)
    {
      // 캐시 버퍼의 아이디가 0이 아닐 경우 해시 테이블에서 페이지 아이디를 제거
    ...
    // 현재 free 상태인 캐시 로그 버퍼가 없을 경우 캐시 버퍼를 확장한다.
    // 캐시 버퍼를 찾지 못한 경우 캐시 로그 버퍼를 확장한다.
...
/* 페이지 버퍼(LA_CACHE_BUFFER)를 반환한다.
만약 페이지 버퍼가 캐시 페이지 버퍼(LA_CACHE_PB)에 존재한다면 해당 페이지 버퍼를 반환하고,
존재하지 않는다면 로그를 새로 읽어 캐시 버퍼를 할당하고 캐시 버퍼를 교체한다.


????? 이때 새로 할당되는 페이지 버퍼의 로그 페이지 사이즈는 4096, 캐시 버퍼 사이즈는 100이다 이다

cache_buffer_size는 사이즈라고 되어있지만 내부에서는 cnt(캐시 페이지 버퍼 당 캐시 로그 버퍼의 수)로 사용하고있고,
cnt * slb_size(single log buffer) 로 계산된다.
 */
static LA_CACHE_BUFFER *
...
/* 페이지의 아이디(pageid)를 이용해 로그 페이지(LOG_PAGE)를 조회한다.
로그 페이지의 정보는 로그 캐시 버퍼(LA_CACHE_BUFFER)자료구조에 포함되어 있으므로,
캐시 버퍼를 먼저 조회해 포인터 주소를 반환한다.*/
static LOG_PAGE *
...
la_release_page_buffer(LOG_PAGEID pageid)
{
  LA_CACHE_PB *cache_pb = la_Info.cache_pb;
  LA_CACHE_BUFFER *cache_buffer = NULL;
  // 해제하려는 페이지 버퍼를 la_Info.cache_pb의 해시 테이블에서 조회한다.
  cache_buffer = (LA_CACHE_BUFFER *)mht_get(cache_pb->hash_table, (void *)&pageid);
  if (cache_buffer != NULL)
  {
    if ((--cache_buffer->fix_count) <= 0)
...
la_decache_page_buffers(LOG_PAGEID from, LOG_PAGEID to)
{
  ...
  for (i = 0; i < cache_pb->num_buffers; i++)
  {
    ...
    if ((cache_buffer->pageid == NULL_PAGEID) || (cache_buffer->pageid == 0) || (cache_buffer->pageid < from) || (cache_buffer->pageid > to))
    {
      continue;
    }
...
la_find_required_lsa(LOG_LSA *required_lsa)
{
  ...
  for (i = 0; i < la_Info.cur_repl; i++)
  {
    ...
    // lowest_lsa 가 NULL 이거나, lowest_lsa가 la_Info.repl_lists[i]->start_lsa보다 크면
    // lowest_lsa에 la_Info.repl_lists[i]->start_lsa를 복사한다.
    //  즉, 복제 리스트 (repl_lists)에서 가장 작은 LSA를 찾는다.
  ...
  // lowest_lsa 가 NULL 이면, la_Info.final_lsa를 required_lsa에 복사하고,
  // 아니면 lowest_lsa(repl_list에서 start_lsa)를 required_lsa에 복사한다.
...
#define LA_OUT_VALUE_COUNT 23
  ...
  snprintf(query_buf, sizeof(query_buf), "SELECT "                   /* SELECT */
                                         ...
                                         "   fail_counter, "         /* 24 */
                                         "   start_time "            /* 25 */
                                         " FROM %s WHERE db_name = ? and copied_log_path = ? ;",
...
la_get_last_ha_applied_info(void)
{
  ...
  if (res > 0)
  {
    ...
    if ((log_db_creation_time.date != apply_info.creation_time.date) || (log_db_creation_time.time != apply_info.creation_time.time))
    {
      return ER_FAILED;
    }
...
la_find_log_pagesize(LA_ACT_LOG *act_log, const char *logpath, const char *dbname, bool check_charset)
{
  ...
  do
  {
    // 로그 볼륨에 있는 page 한덩어리를 가져와 hdr_path 에 저장한다.
    // 이 페이지는 header 와 data의 구성으로 된 페이지이며, 이 중 data의 내용이 log_hdr(LOG_HEADER) 이다.
    // log_hdr은 로그 볼륨의 메타 데이터 역할을 하며, 매직넘버, 생성시간, 삭제 여부, 페이지크기 등을 포함한다.
    error =
    ...
    if (act_log->log_hdr->prefix_name[0] != '\0' && strncmp(act_log->log_hdr->prefix_name, dbname, strlen(dbname)) != 0)
    {
      la_applier_need_shutdown = true;
  ...
  // act_log->log_hdr->db_iopagesize 를 act_log->db_iopagesize 저장한다.
  // 같은 act_log 구조체 내이긴한데 접근 시간때문에 다시하는건가?????
...
la_does_page_exist(LOG_PAGEID pageid)
{
  ...
  if (log_buffer != NULL)
  {
    if (log_buffer->pageid == pageid && log_buffer->logpage.hdr.logical_pageid == pageid && log_buffer->logpage.hdr.offset > NULL_OFFSET)
    {
...
// 로그 페이지의 내용을 rec_type와 area에 복사하는 함수
static void
                    ...
                    LOG_PAGE *log_pgptr)
{
  ...
  // 로그 페이지의 데이터 영역에서 로그의 위치(area + log_offset)의 값을 area + area_offset에 복사한다.
  // area_offset 이 0이고, copy_length만큼 복사하고, area_offset을 copy_length만큼 증가시킨다.
  // 또한 t_length를 copy_length만큼 감소시키며, 데이터가 남아있다면(t_length > 0) 반복한다.
  while (t_length > 0)
  {
    LA_LOG_READ_ADVANCE_WHEN_DOESNT_FIT(error, 0, log_offset, log_pageid, pg);
    if (pg == NULL)
...
la_make_room_for_mvcc_delid_and_prev_ver(RECDES *recdes)
{
  ...
  assert((mvcc_flag & OR_MVCC_FLAG_VALID_INSID) && (mvcc_flag & OR_MVCC_FLAG_VALID_DELID) && (mvcc_flag & OR_MVCC_FLAG_VALID_PREV_VERSION));
 ...
 */
int la_get_undoredo_diff(LOG_PAGE **pgptr, LOG_PAGEID *pageid, PGLENGTH *offset, bool *is_undo_zip, char **undo_data,
                         int *undo_length)
{
  int error = NO_ERROR;
                ...
                unsigned int *rcvindex, void **logs, char **rec_type, char **data, int *d_length)
{
  ...
  {
    /* Get Redo Data */
    // 로그 페이지의 내용을 rec_type(la_Info) 과 data(recdes->data)로 복사한다
...
la_repl_add_object(MOP classop, LA_ITEM *item, RECDES *recdes)
{
  ...
  case RVREPL_DATA_UPDATE:
    // update 시 opertaion 은 LC_FLUSH_DELETE
    operation = LC_UPDATE_OPERATION_TYPE(pruning_type);
    break;
  case RVREPL_DATA_INSERT:
    // insert 시 operation 은 LC_FLUSH_INSERT
...
la_apply_delete_log(LA_ITEM *item)
{
  ...
  {
    /* get class info */
    // 클래스(DB_OBJECT)를 패치해 MOBJ타입의 구조체에 넣는다.
    // 이 코드는 if (la_enable_sql_logging) 아래로 가도 될듯
    ...
    // delete의 경우 item_type이 RVREPL_DATA_DELETE이기 때문에
    // operation 은 LC_FLUSH_DELETE가 된다.
    // 그리고 description은 NULL이 들어간다.
    error = la_repl_add_object(class_obj, item, NULL);
    if (error == NO_ERROR)
    {
      // insert, update 와는 다르게 delete_count를 증가시킴
      la_Info.delete_counter++;
 ...
 */
// la_apply_insert_log와 전반적으로 유사한데 pruning type에 따른 operation 연산,
// repl list 에 대한 update_counter증가 다름
static int
la_apply_update_log(LA_ITEM *item)
{
  ...
  if (rcvindex != RVHF_UPDATE && rcvindex != RVOVF_CHANGE_LINK && rcvindex != RVHF_MVCC_INSERT && rcvindex != RVHF_UPDATE_NOTIFY_VACUUM && rcvindex != RVHF_INSERT_NEWHOME)
  {
  ...
  // 전역변수 repl 에 오브젝트(클래스, 아이템, recdes)를 추가한다.
...
la_apply_insert_log(LA_ITEM *item)
{
  ...
  // 미리 할당된 recdes 배열을 가져온다.
  // 이때 recdes-> data는 풀의 area 영역의 특정 주소를 가리키게 되는데,
  // 가리키는 주소는 pool.area(풀의 시작 주소) + pagesize * recdec.index 로 계산된다.
  recdes = la_assign_recdes_from_pool();
  // mvcc 도 확인할것
  is_mvcc_class = la_is_mvcc_class(ws_oid(class_obj));

  /* retrieve the target record description */
  // 로그 페이지의 내용을 가져와서 recdes->data와 la_Info.rec_type에 저장한다.
...
la_apply_statement_log(LA_ITEM *item)
{
  ...
  case CUBRID_STMT_UPDATE:
    ...
    // *INDENT-OFF*
    if (item->item_type == CUBRID_STMT_ALTER_STORED_PROCEDURE || item->item_type == CUBRID_STMT_DROP_STORED_PROCEDURE || item->item_type == CUBRID_STMT_CREATE_USER || item->item_type == CUBRID_STMT_ALTER_USER || item->item_type == CUBRID_STMT_DROP_USER)
    {
      need_set_user = false;
    }
    // *INDENT-ON*
...
la_get_next_repl_item_from_log(LA_ITEM *item, LOG_LSA *last_lsa)
{
  ...
  while (!LSA_ISNULL(&curr_lsa))
  {
    ...
    {
      if (LSA_GT(&curr_lsa, last_lsa) || curr_log_record->type == LOG_COMMIT || curr_log_record->type == LOG_ABORT || LSA_GE(&curr_lsa, &la_Info.act_log.log_hdr->eof_lsa))
      {
        break;
      }
...
la_log_record_process(LOG_RECORD_HEADER *lrec, LOG_LSA *final, LOG_PAGE *pg_ptr)
{
  ...
  if ((lrec->type != LOG_END_OF_LOG && lrec->type != LOG_DUMMY_HA_SERVER_STATE) && lrec->trid != LOG_SYSTEM_TRANID && LSA_ISNULL(&lrec->prev_tranlsa))
  {
...
la_change_state(void)
{
  ...
  if (la_Info.last_server_state == la_Info.act_log.log_hdr->ha_server_state && la_Info.last_file_state == la_Info.act_log.log_hdr->ha_file_status && la_Info.last_is_end_of_record == la_Info.is_end_of_record)
  {
  ...
  else
  {
    ...
    case HA_SERVER_STATE_ACTIVE:
    case HA_SERVER_STATE_TO_BE_STANDBY:
      if (la_Info.apply_state != HA_LOG_APPLIER_STATE_WORKING && la_Info.apply_state != HA_LOG_APPLIER_STATE_RECOVERING)
      {
    ...
    case HA_SERVER_STATE_DEAD:
    case HA_SERVER_STATE_MAINTENANCE:
      if (la_Info.apply_state != HA_LOG_APPLIER_STATE_DONE && la_Info.apply_state != HA_LOG_APPLIER_STATE_RECOVERING)
      {
        new_state = HA_LOG_APPLIER_STATE_RECOVERING;
      }
      break;
 ...
 */
// slave에 로그 커밋 함수.
// 로그 플러시(NET_SERVER_LC_REPL_FORCE)와 커밋요청(NET_SERVER_TM_SERVER_COMMIT)으로 이뤄짐
static int
la_log_commit(bool update_commit_time)
{
  int res;
  int error = NO_ERROR;
  // 가장 낮은 lsa를 찾아 la_Info.required_lsa에 저장
  (void)la_find_required_lsa(&la_Info.required_lsa);

  // la_Info.act_log.log_hdr->append_lsa 관련 찾을때 다시 봐야함
                            ...
                            bool verbose, LOG_LSA *applied_final_lsa)
{
  ...
  if ((res <= 0) || (ha_apply_info.creation_time.date == 0 && ha_apply_info.creation_time.time == 0))
  {
    ...
    {
      error = ER_FAILED;
      PRINT_AND_LOG_ERR_MSG(msgcat_message(MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_APPLYINFO, APPLYINFO_MSG_NO_QUERY_RESULTS));
    }
 ...
 */
// 아카이브 로그를 삭제하고 마지막으로 삭제된 아카이브 넘버를 반환한다.
// poc 에서는 주석처리하고, 나중에는 현재 참조하는 archive 로그 파일 찾는 방식을 다르게 해야할수도
int la_remove_archive_logs(const char *db_name, int last_deleted_arv_num, int nxarv_num, int max_arv_count_to_delete)
{
  ...
  // 최대 archive 로그의 수를 구한다. 
  
  // 그렇지 않다면 설정된 copy_log_max_archives 수를 사용한다.
  if (LA_LOG_IS_IN_ARCHIVE(la_Info.required_lsa.pageid))
  {
    // pageid 가 archive 로그에 해당하는 경우, la_find_archive_num 함수를 필요한 arv 수(required_arv_num)을  구하고,
    // nxarv_num - required_arv_num???????
  ...
  // 현재 아카이브 로그 수를 구한다. 
  // 다음 아카이브가 10, 마지막으로 삭제된 아카이브가 5라면, 현재 아카이브 로그 수는 10 - 5 - 1 = 4 가 현재 아카이브가 된다.
  // 마지막 1은 현재 아카이브 로그를 포함하지 않기 위해서??????
  current_arv_count = nxarv_num - last_deleted_arv_num - 1;
  if (current_arv_count > max_arv_count)
  {
    // 삭제 범위의 시작은 마지막으로 삭제된 아카이브 + 1
    // 마지막은 다음 아카이브 넘버 - 최대 아카이브 수 -1 
    // 위에서 nxarv_num 이 3, required_arv_num 이 2이라면, log_max_archives 가 0, last가 1이라고 가정. 
    ...
    // first_arv_num_to_delete 부터 last_arv_num_to_delete 까지의 아카이브 로그를 삭제한다.
...
la_need_filter_out(LA_ITEM *item)
{
  ...
  if (filter->type == REPL_FILTER_NONE || (item->log_type == LOG_REPLICATION_STATEMENT && item->item_type != CUBRID_STMT_TRUNCATE) || strcasecmp(class_name, CT_SERIAL_NAME) == 0)
  {
    return false;
  }
  ...
  if ((filter->type == REPL_FILTER_INCLUDE_TBL && filter_found == false) || (filter->type == REPL_FILTER_EXCLUDE_TBL && filter_found == true))
  {
    return true;
  }
...
/*
static int
check_reinit_copylog2(void)
{
  if (!la_fetch_log_hdr(&la_Info.act_log))
    return ER_FAILED;
...
int la_apply_log_file(const char *database_name, const char *log_path, const int max_mem_size)
{
  ...
  /* init la_Info */
  // path /CUBRID-11..../databases/demodb_youngjun2
  // 파일 디스크립터들은 -1로 설정되고 아래 la_find_log_pagesize 에서 파일을 열어 디스크립터에 저장된다.
  ...
  // 이름은 log_pagesize 인데, 파일 이름을 만들고, 파일을 열고, validation 후 페이지 사이즈를 가져온다.
  // 주석으로는 시작 페이지 Id도 가지고 온다는데 실제로는 보이지 않는듯하다.
  // act log 의 suffix 는 log _lgat이다.
  ...
  /* get log info path */
  // 위치가 좀 뜬금없긴해...
  // log info 의 suffix 는 _lginf이다.
  // ha apply info 가 호출될때마다 파일 쓰기가 발생하는데 특별한 데이터는 쓰여지지 않고,
  // 읽는 작업은 하지 않음
  ...
  do
  {
    ...
    while (!LSA_ISNULL(&la_Info.final_lsa) && la_applier_need_shutdown == false)
    {
      ...
      // final_lsa.pageid는 캐시 버퍼가 아닌 디스크로부터 읽어와야하기때문에 모든 캐시를 비운다.
      // 왜????
      la_decache_page_buffers(la_Info.final_lsa.pageid, LOGPAGEID_MAX);

      // copylog가 다시 초기화 됐는지 확인한다.
      // mark_will_del 면 reinit된 copylog 이고, error가 failed 이므로 applier는 종료된다.
      error = check_reinit_copylog();
      if (error != NO_ERROR)
      {
        la_applier_need_shutdown = true;
      ...
      {
        if (final_log_hdr.ha_server_state == HA_SERVER_STATE_DEAD && LSA_EQ(&last_eof_lsa, &final_log_hdr.eof_lsa))
        {
          now = time(NULL);
      ...
      if (la_Info.apply_state == HA_LOG_APPLIER_STATE_DONE && (final_log_hdr.ha_server_state != HA_SERVER_STATE_ACTIVE) && (final_log_hdr.ha_server_state != HA_SERVER_STATE_TO_BE_STANDBY))
      {
        ...
        if (LSA_GE(&la_Info.final_lsa, &la_Info.committed_lsa))
        {
          ...
          if (error == ER_NET_CANT_CONNECT_SERVER || error == ER_OBJ_NO_CONNECT || error == ER_LC_PARTIALLY_FAILED_TO_FLUSH || error == ER_LC_FAILED_TO_FLUSH_REPL_ITEMS)
          {
            la_shutdown();
            return error;
          }
      ...
      if (log_buf->logpage.hdr.logical_pageid == la_Info.final_lsa.pageid)
      {
        if (log_buf->logpage.hdr.offset < 0)
        {
          ...
          if ((final_log_hdr.ha_file_status == LOG_HA_FILESTAT_SYNCHRONIZED) && ((la_Info.final_lsa.pageid + 1) <= final_log_hdr.eof_lsa.pageid) && (la_does_page_exist(la_Info.final_lsa.pageid + 1) != LA_PAGE_DOESNOT_EXIST))
          {
            ...
            if (la_does_page_exist(la_Info.final_lsa.pageid) == LA_PAGE_DOESNOT_EXIST && la_Info.final_lsa.pageid < final_log_hdr.eof_lsa.pageid)
            {
      ...
      while (la_Info.final_lsa.pageid == log_buf->pageid && la_applier_need_shutdown == false)
      {
        ...
        // log page{log hdrpage(logcal_pageid | offset | flags | checksum) | area(log record header | data header | data) |} | log record | ...
        // 페이지 내에서 lsa offset(각 로그의 위치)들로 로그 레코드를 접근.
        // 로그 페이지는 로그 헤더와 area구성되고, area는 다시 로그 레코드 헤더와 데이터 헤더, 데이터로 구성
      ...
      if (la_Info.final_lsa.pageid >= final_log_hdr.eof_lsa.pageid || la_Info.final_lsa.pageid >= final_log_hdr.append_lsa.pageid || la_Info.is_end_of_record == true)
      {
