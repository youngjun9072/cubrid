...
start:
	...
	else if (entry != NULL)
	{
		...
		if (entry->e_tran_index == tran_index)
		{
			 ...
			 * involved.
			 */
			if (entry->e_current.action == LC_CLASSNAME_DELETED || entry->e_current.action == LC_CLASSNAME_DELETED_RENAME || (entry->e_current.action == LC_CLASSNAME_RESERVED && LSA_ISNULL(&entry->e_current.savep_lsa)))
			{
				/*
			...
			{
				assert((entry->e_current.action == LC_CLASSNAME_RESERVED && !LSA_ISNULL(&entry->e_current.savep_lsa)) || entry->e_current.action == LC_CLASSNAME_RESERVED_RENAME);
...
start:
	...
	else if (entry != NULL)
	{
		...
		if (entry->e_tran_index == tran_index)
		{
			...
			assert(entry->e_current.action == LC_CLASSNAME_RESERVED || entry->e_current.action == LC_CLASSNAME_RESERVED_RENAME);
...
locator_drop_class_name_entry(THREAD_ENTRY *thread_p, const char *classname, LOG_LSA *savep_lsa)
{
	...
	if (locator_is_exist_class_name_entry(thread_p, entry) == false)
	{
		...
		if (savep_lsa == NULL && tdes->state != TRAN_UNACTIVE_ABORTED)
		{
			...
			if (entry->e_current.action == LC_CLASSNAME_RESERVED || entry->e_current.action == LC_CLASSNAME_RESERVED_RENAME)
			{
				entry->e_tran_index = NULL_TRAN_INDEX;
			...
			{
				assert(entry->e_current.action == LC_CLASSNAME_DELETED || entry->e_current.action == LC_CLASSNAME_DELETED_RENAME);
	...
	if (locator_is_exist_class_name_entry(thread_p, entry))
	{
		...
		if (disk_is_page_sector_reserved_with_debug_crash(thread_p, class_oid.volid, class_oid.pageid, true) != DISK_VALID)
		{
			assert(false);
		}
	}
...
locator_check_class_on_heap(const void *name, void *ent, void *args)
{
	...
	if (heap_get_class_name_alloc_if_diff(thread_p, class_oid, (char *)classname, &heap_classname) != NO_ERROR || heap_classname == NULL)
	{
 ...
 *              newly assigned OID.
 */
int xlocator_assign_oid(THREAD_ENTRY *thread_p, const HFID *hfid, OID *perm_oid, int expected_length, OID *class_oid,
						const char *classname)
{
	int error_code = NO_ERROR;
...
locator_find_lockset_missing_class_oids(THREAD_ENTRY *thread_p, LC_LOCKSET *lockset)
{
	...
	if (lockset->length < i || lockset->classes != ((LC_LOCKSET_CLASSOF *)(lockset->mem + sizeof(*lockset))) || lockset->objects < ((LC_LOCKSET_REQOBJ *)(lockset->classes + lockset->num_reqobjs)))
	{
 ...
 */
int xlocator_fetch(THREAD_ENTRY *thread_p, OID *oid, int chn, LOCK lock,
				   ...
				   OID *class_oid, int class_chn, int prefetching, LC_COPYAREA **fetch_area)
{
	...
	assert(skip_fetch_version_type_check || (OID_EQ(class_oid, oid_Root_class_oid)) || ((lock != NULL_LOCK) || (lock_get_object_lock(oid, class_oid) != NULL_LOCK) || ((class_lock = lock_get_object_lock(class_oid, oid_Root_class_oid)) == S_LOCK || class_lock >= SIX_LOCK) || ((class_lock = lock_get_object_lock(oid_Root_class_oid, NULL)) == S_LOCK || class_lock >= SIX_LOCK)));
 ...
 */
int xlocator_get_class(THREAD_ENTRY *thread_p, OID *class_oid, int class_chn, const OID *oid, LOCK lock,
 ...
 *
 */
int xlocator_fetch_all(THREAD_ENTRY *thread_p, const HFID *hfid, LOCK *lock, LC_FETCH_VERSION_TYPE fetch_version_type,
					   ...
					   int request_pages)
{
	...
	if (OID_ISNULL(last_oid))
	{
		...
		if (*lock != NULL_LOCK && lock_object(thread_p, class_oid, oid_Root_class_oid, *lock, LK_UNCOND_LOCK) != LK_GRANTED)
		{
			/*
			 * Unable to acquired lock
			 */
...
int xlocator_fetch_lockset(THREAD_ENTRY *thread_p, LC_LOCKSET *lockset, LC_COPYAREA **fetch_area)
{
	...
	*fetch_area = NULL;
	...
	while (scan == S_SUCCESS && ((lockset->num_classes_of_reqobjs_processed < lockset->num_classes_of_reqobjs) || (lockset->num_reqobjs_processed < lockset->num_reqobjs)))
	{
		...
		if (i >= lockset->num_classes_of_reqobjs)
		{
			...
			{
				if (OID_ISNULL(&reqobjs[i].oid) || OID_ISTEMP(&reqobjs[i].oid) || reqobjs[i].class_index == -1 || OID_ISNULL(&reqclasses[reqobjs[i].class_index].oid))
				{
	...
	if ((lockset->num_classes_of_reqobjs_processed >= lockset->num_classes_of_reqobjs) && lockset->num_reqobjs_processed >= lockset->num_reqobjs)
	{
		lock_unlock_objects_lock_set(thread_p, lockset);
	}
 ...
 *
 */
int xlocator_fetch_all_reference_lockset(THREAD_ENTRY *thread_p, OID *oid, int chn, OID *class_oid, int class_chn,
 ...
 */
int xlocator_does_exist(THREAD_ENTRY *thread_p, OID *oid, int chn, LOCK lock, LC_FETCH_VERSION_TYPE fetch_version_type,
 ...
 *   op_type(in):
 */
int locator_start_force_scan_cache(THREAD_ENTRY *thread_p, HEAP_SCANCACHE *scan_cache, const HFID *hfid,
 ...
 */
int locator_insert_force(THREAD_ENTRY *thread_p, HFID *hfid, OID *class_oid, OID *oid, RECDES *recdes, int has_index,
						 ...
						 bool dont_check_fk, bool use_bulk_logging)
{
  ...
  OID rep_dir = { NULL_PAGEID, NULL_SLOTID, NULL_VOLID };
	...
	*force_count = 0;
	 ...
	 */
	// 새 객체. 객체는 oid 가 할당된 후에 반드시 exclusive 모드로 lock 되어야 한다.	
	// 우리는 클래스에 대해서만 한다. 새 instance 는 다른 client 가 어떤 방법으로도 그들에게 도달할 수 없기 때문에 lock 하지 않는다.
	// client 가 oid 를 알 수 있는 방법이 없다.
	 ...
	 * heap_insert_logical */
	// recdes type 을 REC_HOME으로 조정(코드가 여기까지 왔따면 REC_HOME 또는 REC_BIGONE 이어야 한다. 는데 왜??????)
	recdes->type = REC_HOME;

	/* prepare context */
	// context 초기 값 설정
	// ?????????????????????
	// 1. 왜 함수내에서 값 초기화를 다 안해주고 일부는 안에서 일부는 밖에서함?
	// 2. 어차피 여기서만 쓸 변수인데 왜 미리 context를 만들어서 파라미터로 안만들고
	// 여기서 만드는건지 모르겠음
  ...
  if (OID_IS_ROOTOID (&real_class_oid))
	...
	if (has_index && locator_add_or_remove_index(thread_p, recdes, oid, &real_class_oid, true, op_type, local_scan_cache, true,
 ...
 */
int locator_delete_force(THREAD_ENTRY *thread_p, HFID *hfid, OID *oid, int has_index, int op_type,
						   ...
						   DB_VALUE *key_value, HEAP_SCANCACHE *force_scancache)
{
	...
	if (obj->operation != LC_FLUSH_DELETE)
	{
		...
		// or(object representaion) 클래스 객체들이 서버로 플러시될 때 카탈로그 매니저를 업데이트 하는데 사용
		// 교체를 왜하지??????
...
int xlocator_repl_force(THREAD_ENTRY *thread_p, LC_COPYAREA *force_area, LC_COPYAREA **reply_area)
{
	...
	// force_area 에서 LC_COPYAREA_MANYOBJS 주소를 가져옴
	mobjs = LC_MANYOBJS_PTR_IN_COPYAREA(force_area);
	// mobjs 에서 첫 오프젝트의 위치를 가져온 후 바로 포인터 주소를 증가해 다음 오프젝트의 위치를 가져옴
	obj = LC_START_ONEOBJ_PTR_IN_COPYAREA(mobjs);
	obj = LC_PRIOR_ONEOBJ_PTR_IN_COPYAREA(obj);
	...
	// reply_area 내의 포인터 주소 값들로 reply_recdes 를 초기화
	LC_RECDES_IN_COPYAREA(*reply_area, &reply_recdes);

	for (i = 0; i < mobjs->num_objs; i++)
	{
		er_clear();
		// for 밖에서 초기화된 obj 를 사용하여 다음 오브젝트의 위치를 가져옴(2번째 오브젝트부터)
		obj = LC_NEXT_ONEOBJ_PTR_IN_COPYAREA(obj);
		// force_area 에서 pkey를 읽어옴
		// 이때 key 값은 force_area 의 obj 의 offset 위치에 저장됨
		packed_key_value_len = locator_repl_get_key_value(&key_value, force_area, obj);
		/// force_area 에서 recdes 값을 가져옴
		// 이때 recdes 는 force_area->mem + obj.offset + packed_key_value_len 위치에 저장됨
		LC_REPL_RECDES_FOR_ONEOBJ(force_area, obj, packed_key_value_len, &recdes);

		// class info 에서 class_oid 를 가져옴
		...
		// 작업에 필요한 정보 준비
		// last represantaion id 로 교체하고 별다른 작업은 안함
		...
		if (error_code == NO_ERROR)
		{
			...
			case LC_FLUSH_INSERT_PRUNE_VERIFY:
				// pruning type 을 가져오는데 insert 의 경우 DB_NOT_PARTITIONED_CLASS 를 반환
...
int xlocator_force(THREAD_ENTRY *thread_p, LC_COPYAREA *force_area, int num_ignore_error, int *ignore_error_list)
{
	...
	for (i = 0; i < mobjs->num_objs; i++)
	{
		...
		if (locator_manyobj_flag_is_set(mobjs, IS_MULTI_UPDATE) && LC_IS_FLUSH_UPDATE(obj->operation) && !OID_EQ(&obj->class_oid, oid_Root_class_oid))
		{
			continue;
		}
 ...
 */
int locator_attribute_info_force(THREAD_ENTRY *thread_p, const HFID *hfid, OID *oid, HEAP_CACHE_ATTRINFO *attr_info,
								 ...
								 UPDATE_INPLACE_STYLE force_update_inplace, RECDES *rec_descriptor, bool need_locking)
{
	...
	case LC_FLUSH_UPDATE_PRUNE_VERIFY:
		...
		else if (HEAP_IS_UPDATE_INPLACE(force_update_inplace) || need_locking == false)
		{
			...
			assert((lock_get_object_lock(oid, &class_oid) >= X_LOCK) || (lock_get_object_lock(&class_oid, oid_Root_class_oid) >= X_LOCK));
		}
		else
		{
 ...
 */
int locator_add_or_remove_index(THREAD_ENTRY *thread_p, RECDES *recdes, OID *inst_oid, OID *class_oid, int is_insert,
									 ...
									 bool skip_checking_fk)
{
	...
	for (i = 0; i < num_btids; i++)
	{
		 ...
		 */
		if (need_replication && index->type == BTREE_PRIMARY_KEY && error_code == NO_ERROR && !LOG_CHECK_LOG_APPLIER(thread_p) && log_does_allow_replication() == true)
		{
			error_code =
							  ...
							  OID **inst_oids, int num_insts, RECDES **recs, DB_LOGICAL *results)
{
	...
	if (or_pred == NULL || class_oid == NULL || recs == NULL || inst_oids == NULL || num_insts <= 0 || results == NULL || or_pred->pred_stream == NULL || btid == NULL)
	{
		assert(false);
		return ER_FAILED;
	}
 ...
 */
int locator_update_index(THREAD_ENTRY *thread_p, RECDES *new_recdes, RECDES *old_recdes, ATTR_ID *att_id, int n_att_id,
    ...
    // primary key index를 찾지 않았고 replication 상황이고, 인덱스 타입이 Pk 일때 pk btid index를 현재 인덱스(i)로 설정 -> i번째 인덱스가 pk임
      ...
      // mvcc환경에서는, 만약 인덱스가 pk이고 fk 이면 해당 Pk인ㄷ게스를 업데이트할 필요가 있다. (단, 이때 fk 제약조건 검사 자체는 생략 가능)
		...
		if (!same_key || do_delete_only || do_insert_only)
		{
			...
			if (!locator_Dont_check_foreign_key && !same_key && index->type == BTREE_PRIMARY_KEY && index->fk && found_btid)
			{
							...
							ATTR_ID *attr_ids, int *atts_prefix_length, const char *btname, bool repair)
{
	...
	while ((scan = heap_next(thread_p, hfid, class_oid, &inst_oid, &record, &scan_cache, COPY)) == S_SUCCESS)
	{
		...
		if ((n_attr_ids == 1 && heap_attrinfo_read_dbvalues(thread_p, &inst_oid, &record, &attr_info) != NO_ERROR) || (key = heap_attrvalue_get_key(thread_p, index_id, &attr_info, &record, &btid_info, &dbvalue, aligned_buf,
	...
	{
		/* search index */
		if (btree_prepare_bts(thread_p, &bt_scan, btid, &isid, &key_val_range, NULL, class_oid, NULL, NULL, false, NULL) != NO_ERROR)
		{
			assert(er_errid() != NO_ERROR);
			break;
		}
								   ...
								   ATTR_ID *attr_ids, const char *btname, bool repair)
{
	...
	for (j = 0; j < num_classes; j++)
	{
		...
		while ((scan = heap_next(thread_p, hfid, class_oid, &inst_oid, &peek, &scan_cache[j], PEEK)) == S_SUCCESS)
		{
			...
			if (index->filter_predicate)
			{
				if (locator_eval_filter_predicate(thread_p, btid, index->filter_predicate, class_oid, &p_inst_oid, 1, &p_peek, &ev_res) != NO_ERROR)
				{
					goto error;
				}
				else if (ev_res != V_TRUE)
...
locator_guess_sub_classes(THREAD_ENTRY *thread_p, LC_LOCKHINT **lockhint_subclasses)
{
	...
	{
		if (OID_ISNULL(&lockhint->classes[i].oid) || OID_ISTEMP(&lockhint->classes[i].oid) || lockhint->classes[i].need_subclasses <= 0)
		{
			/*
 ...
 *   fk_name(in):
 */
int xlocator_check_fk_validity(THREAD_ENTRY *thread_p, OID *cls_oid, HFID *hfid, TP_DOMAIN *key_type, int n_attrs,
 ...
 *
 */
int xlocator_lock_and_fetch_all(THREAD_ENTRY *thread_p, const HFID *hfid, LOCK *instance_lock,
								...
								MVCC_SNAPSHOT *mvcc_snapshot)
{
	...
	*nfailed_instance_locks = 0;
	...
	if (OID_ISNULL(last_oid))
	{
		...
		/* Obtain the desired lock for the class scan */
		if (*class_lock != NULL_LOCK && lock_object(thread_p, class_oid, oid_Root_class_oid, *class_lock, LK_UNCOND_LOCK) != LK_GRANTED)
		{
			/*
			 * Unable to acquired lock
			 */
...
locator_lock_and_get_object_internal(THREAD_ENTRY *thread_p, HEAP_GET_CONTEXT *context, LOCK lock_mode)
{
	...
	if (!mvcc_is_mvcc_disabled_class(context->class_oid_p))
	{
		...
		if (logtb_find_current_isolation(thread_p) > TRAN_READ_COMMITTED && logtb_check_class_for_rr_isolation_err(context->class_oid_p))
		{
								 ...
								 RECDES *recdes, UPDDEL_MVCC_COND_REEVAL *mvcc_cond_reeval, bool is_upddel)
{
	...
	if (mvcc_cond_reeval->rest_attrs->num_attrs != 0)
	{
		...
		if (fetch_val_list(thread_p, mvcc_cond_reeval->rest_regu_list, NULL, cls_oid, (OID *)oid_inst, NULL, PEEK) != NO_ERROR)
		{
			ev_res = V_ERROR;
			goto end;
		}
	...
	if (mvcc_cond_reeval->range_filter.scan_pred != NULL || mvcc_cond_reeval->key_filter.scan_pred != NULL || mvcc_cond_reeval->data_filter.scan_pred != NULL)
	{
		/* evaluate conditions */
...
locator_decide_operation_type(LOCK lock_mode, LC_FETCH_VERSION_TYPE fetch_version_type)
{
	...
	if (lock_mode > NULL_LOCK && lock_mode <= S_LOCK && (fetch_version_type == LC_FETCH_MVCC_VERSION || fetch_version_type == LC_FETCH_CURRENT_VERSION))
	{
...
// *INDENT-OFF*
int locator_multi_insert_force(THREAD_ENTRY *thread_p, HFID *hfid, OID *class_oid,
