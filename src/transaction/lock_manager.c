...
#define LK_IS_LOCKWAIT_THREAD(thrd) \
  ((thrd)->lockwait != NULL && (thrd)->lockwait_state == (int)LOCK_SUSPENDED)
...
struct lk_global_data
{
  ...
  // *INDENT-OFF*
  lk_global_data()
      : max_obj_locks(0), m_obj_hash_table{}, obj_free_entry_list LF_FREELIST_INITIALIZER
        ,
        num_trans(0), tran_lock_table(NULL), DL_detection_mutex PTHREAD_MUTEX_INITIALIZER
        ,
        last_deadlock_run{0, 0}, TWFG_node(NULL), TWFG_edge(NULL), max_TWFG_edge(0), TWFG_free_edge_idx(0), global_edge_seq_num(0), no_victim_case_count(0), verbose_mode(false), deadlock_and_timeout_detector{0}
#if defined(LK_DUMP)
        ,
        dump_level(0)
#endif
...
#define LK_SET_STANDALONE_XLOCK(lock)                                                    \
  do                                                                                     \
  {                                                                                      \
    if ((lock) == SCH_M_LOCK || (lock) == X_LOCK || lock == IX_LOCK || lock == SIX_LOCK) \
    {                                                                                    \
...
set_error:
  ...
  if (entry_ptr->thrd_entry != NULL && ((entry_ptr->thrd_entry->lockwait_state == LOCK_RESUMED_DEADLOCK_TIMEOUT) || (entry_ptr->thrd_entry->lockwait_state == LOCK_RESUMED_ABORTED_OTHER)))
  {
  ...
  case LOCK_RESOURCE_CLASS:
    ...
    else
    {
      ...
      if (entry_ptr->res_head->key.type == LOCK_RESOURCE_CLASS && OID_IS_VIRTUAL_CLASS_OF_DIR_OID(&entry_ptr->res_head->key.oid))
      {
...
lock_escalate_if_needed(THREAD_ENTRY *thread_p, LK_ENTRY *class_entry, int tran_index)
{
  ...
  if (class_entry->granted_mode == NULL_LOCK || class_entry->granted_mode == S_LOCK || class_entry->granted_mode == X_LOCK || class_entry->granted_mode == SCH_M_LOCK)
  {
                                       ...
                                       LOCK lock)
{
  ...
  if (entry_ptr == NULL)
  {
    assert(lock >= NULL_LOCK && res_ptr->total_waiters_mode >= NULL_LOCK && res_ptr->total_holders_mode >= NULL_LOCK);
...
start:
  ...
  if (class_oid != NULL && !OID_IS_ROOTOID(class_oid))
  {
    ...
    if (ret_val == LK_GRANTED && lock_is_class_lock_escalated(lock_get_object_lock(class_oid, oid_Root_class_oid), lock) == true)
    {
...
lock_tran_lk_entry:
  ...
  if (new_mode == entry_ptr->granted_mode)
  {
    ...
    if (is_instant_duration)
    {
      ...
      if ((lock >= IX_LOCK && (entry_ptr->instant_lock_count == 0 && entry_ptr->granted_mode >= IX_LOCK)) && compat1 != LOCK_COMPAT_YES)
      {
...
lock_select_deadlock_victim(THREAD_ENTRY *thread_p, int s, int t)
{
  ...
  for (v = s; v != t;)
  {
    ...
    {
      if (TWFG_node[v].checked_by_deadlock_detector == false || TWFG_node[v].thrd_wait_stime == 0 || (TWFG_node[v].thrd_wait_stime > TWFG_edge[TWFG_node[v].current].edge_wait_stime))
      {
        /* old transaction, not lockwait state, or incorrect WFG edge */
        /* remove all outgoing edges */
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
             ...
             */
            if ((victims[victim_count].can_timeout == false && can_timeout == true) || (victims[victim_count].can_timeout == can_timeout && (LK_ISYOUNGER(tranid, victims[victim_count].tranid))))
            {
              victim_tranid = tranid;
            }
          }
  ...
  else
  {
    ...
    for (i = 0, v = s; i < num_tran_in_cycle; v = TWFG_node[v].ancestor, i++)
    {
      ...
      if (TWFG_node[next_node].checked_by_deadlock_detector == false || TWFG_node[next_node].thrd_wait_stime == 0 || TWFG_node[next_node].thrd_wait_stime > TWFG_edge[TWFG_node[next_node].current].edge_wait_stime)
      {
                                                       ...
                                                       size_t &suspend_count)
{
  ...
  if (thread_ref.m_status == cubthread::entry::status::TS_DEAD || thread_ref.m_status == cubthread::entry::status::TS_FREE)
  {
    return;
  }
...
int lock_object(THREAD_ENTRY *thread_p, const OID *oid, const OID *class_oid, LOCK lock, int cond_flag)
{
  ...
  {
    // cond_flag != LK_COND_LOCK 이기 때문에 이 로직을 타고
    // transaction descriptor 내에 있는 wait time 을 반환
  ...
  else
  {
    if (old_class_lock < new_class_lock)
    {
      if (class_entry != NULL && class_entry->class_entry != NULL && !OID_IS_ROOTOID(&class_entry->class_entry->res_head->key.oid))
      {
 ...
 *
 */
int lock_object_wait_msecs(THREAD_ENTRY *thread_p, const OID *oid, const OID *class_oid, LOCK lock, int cond_flag,
                           int wait_msecs)
{
#if !defined(SERVER_MODE)
  LK_SET_STANDALONE_XLOCK(lock);
...
int lock_classes_lock_hint(THREAD_ENTRY *thread_p, LC_LOCKHINT *lockhint)
{
  ...
  {
    if (lockhint->classes[i].lock == SCH_M_LOCK || lockhint->classes[i].lock == X_LOCK || lockhint->classes[i].lock == IX_LOCK || lockhint->classes[i].lock == SIX_LOCK)
    {
      lk_Standalone_has_xlock = true;
      break;
    }
...
bool lock_has_lock_transaction(int tran_index)
{
  ...
  tran_lock = &lk_Gl.tran_lock_table[tran_index];
  rv = pthread_mutex_lock(&tran_lock->hold_mutex);
  if (tran_lock->root_class_hold != NULL || tran_lock->class_hold_list != NULL || tran_lock->inst_hold_list != NULL || tran_lock->non2pl_list != NULL)
  {
    lock_hold = true;
  }
  else
 ...
 */
void lock_notify_isolation_incons(THREAD_ENTRY *thread_p, bool (*fun)(const OID *class_oid, const OID *oid, void *args),
                                  void *args)
{
#if !defined(SERVER_MODE)
  return;
...
lock_detect_local_deadlock(THREAD_ENTRY *thread_p)
{
  ...
  for (k = 1; k < lk_Gl.num_trans; k++)
  {
    ...
    for (; s != -2;)
    {
      if (TWFG_node[s].checked_by_deadlock_detector == false || TWFG_node[s].thrd_wait_stime == 0 || (TWFG_node[s].current != -1 && (TWFG_node[s].thrd_wait_stime > TWFG_edge[TWFG_node[s].current].edge_wait_stime)))
      {
        /* A new transaction started */
...
int lock_finalize_composite_lock(THREAD_ENTRY *thread_p, LK_COMPOSITE_LOCK *comp_lock)
{
  ...
  for (lockcomp_class = lockcomp->class_list; lockcomp_class != NULL; lockcomp_class = lockcomp_class->next)
  {
    if (IS_WRITE_EXCLUSIVE_LOCK(lockcomp_class->class_lock_ptr->granted_mode) || lockcomp_class->num_inst_oids == prm_get_integer_value(PRM_ID_LK_ESCALATION_AT))
    {
