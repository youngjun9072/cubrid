...
{
  return (hfid1_p->vfid.fileid == hfid2_p->vfid.fileid && hfid1_p->vfid.volid == hfid2_p->vfid.volid && hfid1_p->hpgid == hfid2_p->hpgid);
}
...
locator_allocate_copy_area_by_length(int min_length)
{
   ...
   */
  // ?????갑자기 튀어나온 locator_Keep.copy_areas에 대해 찾아야함
...
// 메모리 마지막 부분에 위치한 LC_COPYAREA_MANYOBJS 구조체를 가리키는 포인터
  mobjs = LC_MANYOBJS_PTR_IN_COPYAREA(copyarea);

  assert(num_objs <= mobjs->num_objs);
  ptr = desc;
  // ptr을 옮겨가며 obj의 내용을 팩해 desc에 넣는다.
...
// 메모리 마지막 부분에 위치한 LC_COPYAREA_MANYOBJS 구조체를 가리키는 포인터
  ...
  {
    // mobjs 구조체 내의 num_objs만큼 반복문을 돌며 각 객체의 정보를 desc 포인터의 내용으로 채워넣음
    for (i = 0, obj = LC_START_ONEOBJ_PTR_IN_COPYAREA(mobjs); i < num_objs;
         i++, obj = LC_NEXT_ONEOBJ_PTR_IN_COPYAREA(obj))
    {
 ...
 */
int locator_send_copy_area(LC_COPYAREA *copyarea, char **contents_ptr, int *contents_length, char **desc_ptr,
                                  ...
                                  int print_rec)
{
  ...
  if (obj->length < 0 && (obj->length != -1 || (obj->operation != LC_FLUSH_DELETE && obj->operation != LC_FETCH_DELETED && obj->operation != LC_FETCH_DECACHE_LOCK && obj->operation != LC_FETCH_VERIFY_CHN)))
  {
    fprintf(out_fp, "Bad length = %d for object num = %d, OID = %d|%d|%d\n", obj->length, obj_index, obj->oid.volid,
            obj->oid.pageid, obj->oid.slotid);
  }
  else if (obj->offset > copyarea->length || (obj->offset < 0 && (obj->offset != -1 || (obj->operation != LC_FLUSH_DELETE && obj->operation != LC_FETCH_DELETED && obj->operation != LC_FETCH_DECACHE_LOCK))))
  {
