...
stran_server_commit_internal(THREAD_ENTRY *thread_p, unsigned int rid, bool retain_lock, bool *should_conn_reset)
{
  ...
  // 트렌젝션 디스크립터 조회가 반복적으로 이뤄진다.
  // 디스크립터 내에 id가 포함되어 있는데 계속 조회하는 이유는?????
  // 그리고 아래 xtran_server_commit 에서는 tranid 가 null 일수 있음에 대한 예외처리가 있는
  // tdes 의 tail_lsa 가 NULL 이 아니면 업데이트
...
{
  OR_ALIGNED_BUF(REL_MAX_RELEASE_LENGTH + (OR_INT_SIZE * 3) + CUB_MAXHOSTNAMELEN)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  ...
  reply_size = (or_packed_string_length(server_release, &strlen1) + (OR_INT_SIZE * 3) + or_packed_string_length(boot_Host_name, &strlen2));
...
void slocator_fetch(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  char *ptr;
  int success;
  OR_ALIGNED_BUF(NET_COPY_AREA_SENDRECV_SIZE + OR_INT_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *desc_ptr = NULL;
  int desc_size;
...
void slocator_get_class(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  int success;
  char *ptr;
  OR_ALIGNED_BUF(NET_COPY_AREA_SENDRECV_SIZE + OR_OID_SIZE + OR_INT_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *desc_ptr = NULL;
  int desc_size;
...
void slocator_fetch_all(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  char *ptr;
  int fetch_version_type;
  OR_ALIGNED_BUF(NET_COPY_AREA_SENDRECV_SIZE + (OR_INT_SIZE * 5) + OR_OID_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *desc_ptr = NULL;
  int desc_size;
...
void slocator_does_exist(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  LC_COPYAREA *copy_area;
  char *ptr;
  OR_ALIGNED_BUF(NET_COPY_AREA_SENDRECV_SIZE + OR_INT_SIZE + OR_OID_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *desc_ptr = NULL;
  int desc_size;
...
void slocator_repl_force(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  csserror = 0;
  // context_size(혹은 최소 길이보다 큰) locator_Keep.copy_areas.areas에서 반환받는다
  // 실패하면 만큼 메모리를 할당해 copy_area를 만든다.
  ...
  if (copy_area)
  {
    ...
    if (num_objs > 0)
    {
      // 오브젝트가 존재하면 packed_desc에 데이터를 받는다.
...
void slocator_fetch_lockset(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  LC_COPYAREA *copy_area;
  LC_LOCKSET *lockset;
  OR_ALIGNED_BUF(NET_SENDRECV_BUFFSIZE + NET_COPY_AREA_SENDRECV_SIZE + OR_INT_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *desc_ptr;
  int desc_size;
  ...
  do
  {
    ...
    first_call = false;
  } while (copy_area && lockset && ((lockset->num_classes_of_reqobjs > lockset->num_classes_of_reqobjs_processed) || (lockset->num_reqobjs > lockset->num_reqobjs_processed)));
...
void slocator_find_class_oid(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  LOCK lock;
  char *ptr;
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_OID_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
...
void slog_find_lob_locator(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_INT_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *ptr;
...
void shf_create(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  char *ptr;
  OID class_oid;
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_HFID_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  int reuse_oid = 0;
...
void sdblink_get_crypt_keys(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  int area_size = -1;
  char *reply, *area, *ptr;
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_INT_SIZE)
...
void stde_get_mk_info(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_INT_SIZE + OR_BIGINT_SIZE + OR_BIGINT_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
...
void stran_server_commit(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  QUERY_ID query_id;
  // 데이터 unpack, {retain_lock | row_count | n_query_ids | query_id}
...
void stran_server_start_topop(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  int success;
  LOG_LSA topop_lsa;
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_LOG_LSA_ALIGNED_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *ptr;
...
void sbtree_add_index(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_BTID_ALIGNED_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
...
void sbtree_load_index(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  TP_DOMAIN *key_type;
  char *ptr;
  OR_ALIGNED_BUF(OR_INT_SIZE * 2 + OR_BTID_ALIGNED_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
...
sbtree_find_unique_internal(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  char *ptr;
  int success;
  OR_ALIGNED_BUF(OR_OID_SIZE + OR_INT_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
...
void sqmgr_prepare_query(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_INT_SIZE + OR_XASL_ID_SIZE)
  a_reply;
  int error = NO_ERROR;
...
void sqmgr_execute_query(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  OR_ALIGNED_BUF(OR_INT_SIZE * 7 + OR_PTR_ALIGNED_SIZE + OR_CACHE_TIME_SIZE)
  a_reply;
                  ...
                  char *queryinfo_string)
{
  ...
  if (info->sql_hash_text == NULL || qmgr_get_sql_id(thread_p, &sql_id, info->sql_hash_text, strlen(info->sql_hash_text)) != NO_ERROR)
  {
    sql_id = NULL;
  }
...
void slocator_find_lockhint_class_oids(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  malloc_size = ((sizeof(char *) + sizeof(LOCK) + sizeof(int) + sizeof(int) + sizeof(OID) + sizeof(int)) * num_classes);
...
void sthread_kill_tran_index(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  success = (xlogtb_kill_tran_index(thread_p, kill_tran_index, kill_user, kill_host, kill_pid) == NO_ERROR) ? NO_ERROR : ER_FAILED;
  if (success != NO_ERROR)
  {
    (void)return_error_to_client(thread_p, rid);
  }
 ...
 */
int xio_send_user_prompt_to_client(THREAD_ENTRY *thread_p, FILEIO_REMOTE_PROMPT_TYPE prompt_id, const char *prompt,
                                   ...
                                   int reprompt_value)
{
  ...
  prompt_length = (or_packed_string_length(prompt, &strlen1) + or_packed_string_length(failure_prompt, &strlen2) + OR_INT_SIZE * 2 + or_packed_string_length(secondary_prompt, &strlen3) + OR_INT_SIZE);
...
{
  OR_ALIGNED_BUF(OR_LOG_LSA_ALIGNED_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  LOG_LSA *lsa;
...
void ssession_find_or_create_session(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  if (id == DB_EMPTY_SESSION || memcmp(server_session_key, xboot_get_server_session_key(), SERVER_SESSION_KEY_SIZE) != 0 || (error = xsession_check_session(thread_p, id)) != NO_ERROR)
  {
    /* not an error yet */
    er_clear();
    /* create new session */
...
void ssession_get_prepared_statement(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  OR_ALIGNED_BUF(OR_INT_SIZE * 2 + OR_XASL_ID_SIZE)
  a_reply;
...
void scdc_start_session(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  cdc_log("%s : max_log_item (%d), extraction_timeout (%d), all_in_cond (%d), num_extraction_user (%d), num_extraction_class (%d)",
...
{
  OR_ALIGNED_BUF(OR_INT_SIZE + OR_LOG_LSA_ALIGNED_SIZE + OR_INT64_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *ptr;
  LOG_LSA start_lsa;
...
{
  OR_ALIGNED_BUF(OR_INT_SIZE * 3 + OR_LOG_LSA_ALIGNED_SIZE)
  a_reply;
  char *reply = OR_ALIGNED_BUF_START(a_reply);
  char *ptr;
  LOG_LSA start_lsa;
...
void sflashback_get_summary(THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  area_size = OR_OID_SIZE * context.num_class + OR_INT64_SIZE + OR_INT64_SIZE + OR_INT_SIZE + (OR_SUMMARY_ENTRY_SIZE_WITHOUT_CLASS + OR_OID_SIZE * context.num_class) * context.num_summary;
