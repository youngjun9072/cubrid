...
css_connection_handler_thread(THREAD_ENTRY *thread_p, CSS_CONN_ENTRY *conn)
{
  ...
  while (thread_p->shutdown == false && conn->stop_talk == false)
  {
    ...
    if (n == 0)
    {
      ...
      if (ha_Server_state == HA_SERVER_STATE_TO_BE_STANDBY && conn->in_transaction == false && css_count_transaction_worker_threads(thread_p, conn->get_tran_index(), conn->client_id) == 0)
      {
 ...
 *   buffer(in):
 */
void css_initialize_server_interfaces(int (*request_handler)(THREAD_ENTRY *thrd, unsigned int eid, int request,
...
#define MAX_CONNECTIONS css_get_max_connections()
  ...
  css_Connection_worker_pool =
      cubthread::get_manager()->create_worker_pool(MAX_CONNECTIONS, MAX_CONNECTIONS, "connection threads", NULL, 1,
                                                   cubthread::is_logging_configured(cubthread::LOG_WORKER_POOL_CONNECTIONS),
  ...
  case HA_SERVER_STATE_TO_BE_STANDBY:
    ...
    else if (whence == FROM_UNREGISTER_CLIENT)
    {
      if (logtb_count_clients(thread_p) == 1)
      {
        ...
        {
          er_log_debug(ARG_FILE_LINE, "css_check_ha_server_state_for_client: "
...
css_check_ha_log_applier_done(void)
{
  ...
  if (i == ha_Server_num_of_hosts && (ha_Server_state == HA_SERVER_STATE_TO_BE_ACTIVE || ha_Server_state == HA_SERVER_STATE_ACTIVE))
  {
    return true;
  }
  return false;
...
css_check_ha_log_applier_working(void)
{
  ...
  for (i = 0; i < ha_Server_num_of_hosts; i++)
  {
    if (ha_Log_applier_state[i].state != HA_LOG_APPLIER_STATE_WORKING || ha_Log_applier_state[i].state != HA_LOG_APPLIER_STATE_DONE)
    {
      break;
    }
  }
  if (i == ha_Server_num_of_hosts && (ha_Server_state == HA_SERVER_STATE_TO_BE_STANDBY || ha_Server_state == HA_SERVER_STATE_STANDBY))
  {
    return true;
  }
  return false;
...
int css_change_ha_server_state(THREAD_ENTRY *thread_p, HA_SERVER_STATE state, bool force, int timeout, bool heartbeat)
{
  ...
  er_log_debug(ARG_FILE_LINE, "css_change_ha_server_state: ha_Server_state %s "
  ...
  if (state == ha_Server_state || (!force && ha_Server_state == HA_SERVER_STATE_TO_BE_ACTIVE && state == HA_SERVER_STATE_ACTIVE) || (!force && ha_Server_state == HA_SERVER_STATE_TO_BE_STANDBY && state == HA_SERVER_STATE_STANDBY))
  {
    return NO_ERROR;
  }

  if (heartbeat == false && !(ha_Server_state == HA_SERVER_STATE_STANDBY && state == HA_SERVER_STATE_MAINTENANCE) && !(ha_Server_state == HA_SERVER_STATE_MAINTENANCE && state == HA_SERVER_STATE_STANDBY) && !(force && ha_Server_state == HA_SERVER_STATE_TO_BE_ACTIVE && state == HA_SERVER_STATE_ACTIVE))
  {
    return NO_ERROR;
  }
...
css_check_accessibility(SOCKET new_fd)
{
  ...
  if (clt_sock_addr.sin_family == AF_UNIX || (ip_addr[0] == 127 && ip_addr[1] == 0 && ip_addr[2] == 0 && ip_addr[3] == 1))
  {
    return NO_ERROR;
  }
...
#if defined(WINDOWS)
  if (strlen(prm_get_string_value(PRM_ID_ACCESS_IP_CONTROL_FILE)) > 2 && isalpha(prm_get_string_value(PRM_ID_ACCESS_IP_CONTROL_FILE)[0]) && prm_get_string_value(PRM_ID_ACCESS_IP_CONTROL_FILE)[1] == ':')
#else
...
void css_server_task::execute(context_type &thread_ref)
{
...
void css_connection_task::execute(context_type &thread_ref)
{
  thread_ref.conn_entry = &m_conn;
...
css_stop_non_log_writer(THREAD_ENTRY &thread_ref, bool &stop_mapper, THREAD_ENTRY &stopper_thread_ref)
{
  ...
  if (thread_ref.m_status == cubthread::entry::status::TS_WAIT && logtb_is_current_active(&thread_ref))
  {
    ...
    if (thread_ref.tran_index != NULL_TRAN_INDEX && thread_ref.m_status == cubthread::entry::status::TS_WAIT && thread_ref.lockwait == NULL && thread_ref.check_interrupt)
    {
                                             ...
                                             size_t &count)
{
  ...
  if (!thread_ref.is_on_current_thread() && thread_ref.m_status != cubthread::entry::status::TS_DEAD && thread_ref.m_status != cubthread::entry::status::TS_FREE && thread_ref.m_status != cubthread::entry::status::TS_CHECK)
  {
...
css_get_connection_thread_timeout_configuration(void)
{
  ...
  return cubthread::wait_seconds(std::chrono::seconds(prm_get_integer_value(PRM_ID_THREAD_CONNECTION_TIMEOUT_SECONDS)));
}
