...
#define HEAP_IS_PAGE_OF_OID(thread_p, pgptr, oid) \
  (((pgptr) != NULL) && pgbuf_get_volume_id(pgptr) == (oid)->volid && pgbuf_get_page_id(pgptr) == (oid)->pageid)
...
#define HEAP_PAGE_SET_VACUUM_STATUS(chain, status)                                                                          \
  do                                                                                                                        \
  {                                                                                                                         \
    assert((status) == HEAP_PAGE_VACUUM_NONE || (status) == HEAP_PAGE_VACUUM_ONCE || (status) == HEAP_PAGE_VACUUM_UNKNOWN); \
...
#define HEAP_PAGE_GET_VACUUM_STATUS(chain)                                                     \
  (((chain)->flags & HEAP_PAGE_FLAG_VACUUM_STATUS_MASK) == 0                                   \
       ...
       : ((((chain)->flags & HEAP_PAGE_FLAG_VACUUM_STATUS_MASK) == HEAP_PAGE_FLAG_VACUUM_ONCE) \
...
#define heap_bestspace_log(...)                   \
  if (prm_get_bool_value(PRM_ID_DEBUG_BESTSPACE)) \
 ...
 */
int xheap_get_maxslotted_reclength(int &maxslotted_reclength)
{
 ...
 */
int heap_classrepr_dump_anyfixed(void)
{
  int ret = NO_ERROR;
...
heap_stats_update_internal(THREAD_ENTRY *thread_p, const HFID *hfid, VPID *lotspace_vpid, int free_space)
{
  ...
  if (free_space >= heap_stats_get_min_freespace(heap_hdr))
  {
    ...
    {
      if (VPID_ISNULL(&heap_hdr->estimates.best[best].vpid) || heap_hdr->estimates.best[best].freespace <= HEAP_DROP_FREE_SPACE)
      {
        break;
      }
...
heap_stats_put_second_best(HEAP_HDR_STATS *heap_hdr, VPID *vpid)
{
  ...
  if (heap_hdr->estimates.num_substitutions++ % 1000 == 0)
  {
    ...
    assert((heap_hdr->estimates.tail_second_best > heap_hdr->estimates.head_second_best)
               ? ((heap_hdr->estimates.tail_second_best - heap_hdr->estimates.head_second_best) == heap_hdr->estimates.num_second_best)
               : ((10 + heap_hdr->estimates.tail_second_best - heap_hdr->estimates.head_second_best) == heap_hdr->estimates.num_second_best));
...
heap_stats_get_second_best(HEAP_HDR_STATS *heap_hdr, VPID *vpid)
{
  ...
  assert((heap_hdr->estimates.tail_second_best >= heap_hdr->estimates.head_second_best)
             ? ((heap_hdr->estimates.tail_second_best - heap_hdr->estimates.head_second_best) == heap_hdr->estimates.num_second_best)
             : ((HEAP_NUM_BEST_SPACESTATS + heap_hdr->estimates.tail_second_best - heap_hdr->estimates.head_second_best) == heap_hdr->estimates.num_second_best));
...
#define BEST_PAGE_SEARCH_MAX_COUNT 100
  ...
  while (found == HEAP_FINDSPACE_NOTFOUND)
  {
    ...
    if (hash_is_available)
    {
      ...
      while (notfound_cnt < BEST_PAGE_SEARCH_MAX_COUNT && (ent = (HEAP_STATS_ENTRY *)mht_get2(heap_Bestspace->hfid_ht, hfid, NULL)) != NULL)
      {
                          ...
                          int newrec_size, HEAP_SCANCACHE *scan_cache, PGBUF_WATCHER *pg_watcher)
{
  ...
  while (true)
  {
    ...
    do
    {
      ...
      heap_bestspace_log("heap_stats_find_best_page: call heap_stats_sync_bestspace() "
                         ...
                         "scan_all %d ",
                         hfid->vfid.fileid, hfid->vfid.volid, hfid->hpgid, hdr_vpidp->pageid,
                         hdr_vpidp->volid, 0);
  ...
  if (pg_watcher->pgptr == NULL)
  {
    ...
    assert(pg_watcher->pgptr != NULL || er_errid() == ER_INTERRUPTED || er_errid() == ER_FILE_NOT_ENOUGH_PAGES_IN_DATABASE);
  }
                          ...
                          bool scan_all, bool can_cycle)
{
  ...
  while (!VPID_ISNULL(&next_vpid) || can_cycle == true)
  {
    ...
    while ((scan_all == true || num_high_best < HEAP_NUM_BEST_SPACESTATS) && !VPID_ISNULL(&next_vpid) && (can_cycle == true || !VPID_EQ(&next_vpid, &stopat_vpid)))
    {
      if (scan_all == false)
      {
    ...
    if (scan_all == false && (iterate_all == true || num_high_best == HEAP_NUM_BEST_SPACESTATS || (can_cycle == false && VPID_EQ(&next_vpid, &stopat_vpid))))
    {
      break;
    }
  ...
  assert(heap_hdr->estimates.head >= 0 && heap_hdr->estimates.head < HEAP_NUM_BEST_SPACESTATS && heap_hdr->estimates.num_high_best <= HEAP_NUM_BEST_SPACESTATS);
...
bool heap_remove_page_on_vacuum(THREAD_ENTRY *thread_p, PAGE_PTR *page_ptr, HFID *hfid)
{
  ...
  if (heap_vpid_prev(thread_p, hfid, *page_ptr, &prev_vpid) != NO_ERROR || heap_vpid_next(thread_p, hfid, *page_ptr, &next_vpid) != NO_ERROR)
  {
    /* Give up. */
    vacuum_er_log_warning(VACUUM_ER_LOG_HEAP,
                          "Could not remove candidate empty heap page %d|%d.", page_vpid.volid, page_vpid.pageid);
 ...
 * Note: Find the next page of heap file.
 */
int heap_vpid_skip_next(THREAD_ENTRY *thread_p, const HFID *hfid, PGBUF_WATCHER *curr_page_watcher,
 ...
 */
int xheap_destroy_newly_created(THREAD_ENTRY *thread_p, const HFID *hfid, const OID *class_oid, const bool force)
{
  VFID vfid;
  FILE_TYPE file_type;
  int ret;
...
int xheap_reclaim_addresses(THREAD_ENTRY *thread_p, const HFID *hfid)
{
  ...
  while (!(VPID_ISNULL(&prv_vpid)))
  {
    ...
    if (spage_number_of_records(curr_page_watcher.pgptr) > 1 || (vpid.pageid == hfid->hpgid && vpid.volid == hfid->vfid.volid))
    {
    ...
    if (!(vpid.pageid == hfid->hpgid && vpid.volid == hfid->vfid.volid) && spage_number_of_records(curr_page_watcher.pgptr) <= 1
...
heap_ovf_insert(THREAD_ENTRY *thread_p, const HFID *hfid, OID *ovf_oid, RECDES *recdes)
{
  ...
  if (heap_ovf_find_vfid(thread_p, hfid, &ovf_vfid, true, PGBUF_UNCONDITIONAL_LATCH) == NULL || overflow_insert(thread_p, &ovf_vfid, &ovf_vpid, recdes, FILE_MULTIPAGE_OBJECT_HEAP) != NO_ERROR)
  {
    return NULL;
  }
 ...
 *
 */
int heap_scancache_start(THREAD_ENTRY *thread_p, HEAP_SCANCACHE *scan_cache, const HFID *hfid, const OID *class_oid,
 ...
 */
int heap_scancache_start_modify(THREAD_ENTRY *thread_p, HEAP_SCANCACHE *scan_cache, const HFID *hfid,
...
try_again:
  ...
  if (context->class_oid_p != NULL && OID_ISNULL(context->class_oid_p) && heap_get_class_oid_from_page(thread_p, context->home_page_watcher.pgptr, context->class_oid_p) != NO_ERROR)
  {
    /* Unexpected. */
    assert_release(false);
    goto error;
  ...
  if (context->fwd_page_watcher.pgptr != NULL && slot_p->record_type != REC_RELOCATION && slot_p->record_type != REC_BIGONE)
  {
  ...
  case REC_MARKDELETED:
    ...
    if (OID_EQ(context->class_oid_p, oid_Root_class_oid) || OID_EQ(context->class_oid_p, oid_User_class_oid) || non_ex_handling_type == LOG_WARNING_IF_DELETED)
    {
...
heap_get_mvcc_header(THREAD_ENTRY *thread_p, HEAP_GET_CONTEXT *context, MVCC_REC_HEADER *mvcc_header)
{
  ...
  assert(context->record_type == REC_HOME || context->record_type == REC_RELOCATION || context->record_type == REC_BIGONE);
  assert(context->record_type == REC_HOME || (forward_page != NULL && pgbuf_get_page_id(forward_page) == context->forward_oid.pageid && pgbuf_get_volume_id(forward_page) == context->forward_oid.volid));
...
heap_get_record_data_when_all_ready(THREAD_ENTRY *thread_p, HEAP_GET_CONTEXT *context)
{
  ...
  assert((context->ispeeking == PEEK) || (context->ispeeking == COPY && (scan_cache_p != NULL || context->recdes_p->data != NULL)));
    ...
    while (true)
    {
      ...
      else
      {
        ...
        while (true)
        {
          ...
          {
            assert(thread_p->_unload_parallel_process_idx >= 0 && thread_p->_unload_parallel_process_idx < thread_p->_unload_cnt_parallel_process);
 ...
 */
int heap_cmp(THREAD_ENTRY *thread_p, const OID *oid, RECDES *recdes)
{
  HEAP_SCANCACHE scan_cache;
  RECDES peek_recdes;
  int compare;
 ...
 */
int heap_scanrange_start(THREAD_ENTRY *thread_p, HEAP_SCANRANGE *scan_range, const HFID *hfid, const OID *class_oid,
                         MVCC_SNAPSHOT *mvcc_snapshot)
{
  int ret = NO_ERROR;
...
heap_scanrange_to_following(THREAD_ENTRY *thread_p, HEAP_SCANRANGE *scan_range, OID *start_oid)
{
  ...
  scan_range->last_oid = scan_range->first_oid;
  if (scan_range->scan_cache.page_watcher.pgptr != NULL && (vpid = pgbuf_get_vpid_ptr(scan_range->scan_cache.page_watcher.pgptr)) != NULL && (vpid->pageid == scan_range->last_oid.pageid) && (vpid->volid == scan_range->last_oid.volid) && spage_get_record_type(scan_range->scan_cache.page_watcher.pgptr, scan_range->last_oid.slotid) == REC_HOME)
  {
    slotid = scan_range->last_oid.slotid;
    while (true)
    {
      if (spage_next_record(scan_range->scan_cache.page_watcher.pgptr, &slotid, &recdes, PEEK) != S_SUCCESS || spage_get_record_type(scan_range->scan_cache.page_watcher.pgptr, slotid) != REC_HOME)
      {
        break;
      }
      else
...
heap_scanrange_to_prior(THREAD_ENTRY *thread_p, HEAP_SCANRANGE *scan_range, OID *last_oid)
{
  ...
  if (scan_range->scan_cache.page_watcher.pgptr != NULL)
  {
    ...
    while (true)
    {
      if (spage_previous_record(scan_range->scan_cache.page_watcher.pgptr, &slotid, &recdes, PEEK) != S_SUCCESS || slotid == HEAP_HEADER_AND_CHAIN_SLOTID || spage_get_record_type(scan_range->scan_cache.page_watcher.pgptr, slotid) != REC_HOME)
      {
        break;
      }
      else
...
bool heap_does_exist(THREAD_ENTRY *thread_p, OID *class_oid, const OID *oid)
{
   ...
   * for it
   */
  if (class_oid != NULL && !OID_EQ(class_oid, oid_Root_class_oid) && HEAP_ISVALID_OID(thread_p, class_oid) != DISK_VALID)
  {
    doesexist = false;
    goto exit_on_end;
  }
 ...
 * returned.
 */
int heap_get_class_name_alloc_if_diff(THREAD_ENTRY *thread_p, const OID *class_oid, char *guess_classname,
                                      char **classname_out)
{
  char *classname = NULL;
  RECDES recdes;
 ...
 */
int heap_attrinfo_start(THREAD_ENTRY *thread_p, const OID *class_oid, int requested_num_attrs, const ATTR_ID *attrids,
                        HEAP_CACHE_ATTRINFO *attr_info)
{
...
heap_attrvalue_read(RECDES *recdes, HEAP_ATTRVALUE *value, HEAP_CACHE_ATTRINFO *attr_info)
{
  ...
  if (recdes == NULL || recdes->data == NULL || value->read_attrepr == NULL || value->attr_type == HEAP_SHARED_ATTR || value->attr_type == HEAP_CLASS_ATTR)
  {
    /*
  ...
  else
  {
    ...
    if (value->read_attrepr->is_fixed != 0)
    {
                                   ...
                                   attr_info->read_classrepr->fixed_length, value->read_attrepr->position))
      {
         ...
         */
        disk_data =
            ((char *)recdes->data + OR_FIXED_ATTRIBUTES_OFFSET_BY_OBJ(recdes->data, attr_info->read_classrepr->n_variable) + value->read_attrepr->location);
 ...
 */
int heap_attrinfo_read_dbvalues(THREAD_ENTRY *thread_p, const OID *inst_oid, RECDES *recdes,
                                HEAP_CACHE_ATTRINFO *attr_info)
{
  int i;
...
int heap_attrinfo_delete_lob(THREAD_ENTRY *thread_p, RECDES *recdes, HEAP_CACHE_ATTRINFO *attr_info)
{
  ...
  for (i = 0; i < attr_info->num_values; i++)
  {
    ...
    if (value->last_attrepr->type == DB_TYPE_BLOB || value->last_attrepr->type == DB_TYPE_CLOB)
    {
      ...
      {
        DB_ELO *elo;
        assert(db_value_type(&value->dbvalue) == DB_TYPE_BLOB || db_value_type(&value->dbvalue) == DB_TYPE_CLOB);
 ...
 */
int heap_get_class_subclasses(THREAD_ENTRY *thread_p, const OID *class_oid, int *count, OID **subclasses)
{
  HEAP_SCANCACHE scan_cache;
  RECDES recdes;
  int error = NO_ERROR;
                                ...
                                HEAP_CACHE_ATTRINFO *attr_info)
{
  ...
  for (i = 0; i < attr_info->num_values; i++)
  {
    ...
    else if (value->state == HEAP_WRITTEN_ATTRVALUE && (value->last_attrepr->type == DB_TYPE_BLOB || value->last_attrepr->type == DB_TYPE_CLOB))
    {
      DB_VALUE *save;
...
resize_and_start:
  ...
  case 0:
    ...
    ptr_varvals = (ptr_bound + OR_BOUND_BIT_BYTES(attr_info->last_classrepr->n_attributes - attr_info->last_classrepr->n_variable));
    ...
    for (i = 0; i < attr_info->num_values; i++)
    {
      ...
      if (value->last_attrepr->is_fixed != 0)
      {
         ...
         */
        buf->ptr = (buf->buffer + OR_FIXED_ATTRIBUTES_OFFSET_BY_OBJ(buf->buffer, attr_info->last_classrepr->n_variable) + value->last_attrepr->location);
      ...
      else
      {
        ...
        if (dbvalue != NULL && db_value_is_null(dbvalue) != true)
        {
          ...
          if (lob_create_flag == LOB_FLAG_INCLUDE_LOB && value->state == HEAP_WRITTEN_ATTRVALUE && (pr_type->id == DB_TYPE_BLOB || pr_type->id == DB_TYPE_CLOB))
          {
            ...
            if (heap_get_class_name(thread_p, &(attr_info->class_oid), &new_meta_data) != NO_ERROR || new_meta_data == NULL)
            {
              status = S_ERROR;
              break;
            }
 ...
 *   idx_info(in):
 */
int heap_attrinfo_start_with_index(THREAD_ENTRY *thread_p, OID *class_oid, RECDES *class_recdes,
...
/* support for SUPPORT_DEDUPLICATE_KEY_MODE */
int heap_get_compress_attr_by_btid(THREAD_ENTRY *thread_p, OID *class_oid, BTID *btid, ATTR_ID *last_attrid,
                       ...
                       TP_DOMAIN **key_domain, OID *rec_oid, bool is_check_foreign)
{
  ...
  *btid = index->btid;
   ...
   */
  if ((n_atts > 1 && recdes != NULL && index->func_index_info == NULL) || (index->func_index_info && (index->func_index_info->attr_index_start + 1) > 1))
  {
    DB_MIDXKEY midxkey;
    int midxkey_size = recdes->length;
    ...
    if (heap_midxkey_key_get(recdes, &midxkey, index, idx_attrinfo, fi_res, fi_domain, key_domain, rec_oid, is_check_foreign) == NULL)
    {
      return NULL;
    }
 ...
 */
int heap_indexinfo_get_attrs_prefix_length(int btid_index, HEAP_CACHE_ATTRINFO *attrinfo, int *attrs_prefix_length,
 ...
 *   btnamepp(in);
 */
int heap_get_indexinfo_of_btid(THREAD_ENTRY *thread_p, const OID *class_oid, const BTID *btid, BTREE_TYPE *type,
 ...
 */
int heap_get_referenced_by(THREAD_ENTRY *thread_p, OID *class_oid, const OID *obj_oid, RECDES *recdes,
                           int *max_oid_cnt, OID **oid_list)
{
  ...
  if ((heap_attrinfo_start_refoids(thread_p, class_oid, &attr_info) != NO_ERROR) || heap_attrinfo_read_dbvalues(thread_p, obj_oid, recdes, &attr_info) != NO_ERROR)
  {
    goto error;
  }
  ...
  for (i = 0; i < attr_info.num_values; i++)
  {
    ...
    if (dbtype == DB_TYPE_OID && !db_value_is_null(&value->dbvalue) && (attr_oid = db_get_oid(&value->dbvalue)) != NULL && !OID_ISNULL(attr_oid))
    {
      /*
    ...
    else
    {
      if (TP_IS_SET_TYPE(dbtype))
      {
        ...
        for (j = 0; j < cnt; j++)
        {
          ...
          if (dbtype == DB_TYPE_OID && !db_value_is_null(&db_value) && (attr_oid = db_get_oid(&db_value)) != NULL && !OID_ISNULL(attr_oid))
          {
            if (oid_cnt == *max_oid_cnt)
            {
              /*
...
int heap_prefetch(THREAD_ENTRY *thread_p, OID *class_oid, const OID *oid, LC_COPYAREA_DESC *prefetch)
{
  ...
  while (direction != HEAP_DIRECTION_NONE)
  {
    ...
    if (direction == HEAP_DIRECTION_LEFT || direction == HEAP_DIRECTION_BOTH)
    {
      ...
      if (scan == S_SUCCESS && left_slotid != HEAP_HEADER_AND_CHAIN_SLOTID && spage_get_record_type(pgptr, left_slotid) == REC_HOME)
      {
        prefetch->mobjs->num_objs++;
        COPY_OID(&((*prefetch->obj)->class_oid), class_oid);
        (*prefetch->obj)->oid.volid = oid->volid;
...
heap_dump_hdr(FILE *fp, HEAP_HDR_STATS *heap_hdr)
{
  ...
  avg_length = ((heap_hdr->estimates.num_recs > 0)
                    ? (int)((heap_hdr->estimates.recs_sumlen / (float)heap_hdr->estimates.num_recs) + 0.9)
                    : 0);
 ...
 */
int heap_dump_heap_file(THREAD_ENTRY *thread_p, FILE *fp, bool dump_records, const char *class_name)
{
  int error_code = NO_ERROR;
  OID class_oid;
 ...
 */
void heap_chnguess_dump(FILE *fp)
{
...
heap_mvcc_log_delete(THREAD_ENTRY *thread_p, LOG_DATA_ADDR *p_addr, LOG_RCVINDEX rcvindex)
{
  ...
  assert(p_addr != NULL);
  assert(rcvindex == RVHF_MVCC_DELETE_REC_HOME || rcvindex == RVHF_MVCC_DELETE_REC_NEWHOME || rcvindex == RVHF_MVCC_DELETE_OVERFLOW);
 ...
 *   npages(in):
 */
int xheap_get_class_num_objects_pages(THREAD_ENTRY *thread_p, const HFID *hfid, int approximation, int *nobjs,
                                      int *npages)
{
  int length, num;
  int ret;
 ...
 */
int heap_set_autoincrement_value(THREAD_ENTRY *thread_p, HEAP_CACHE_ATTRINFO *attr_info, HEAP_SCANCACHE *scan_cache,
                                 int *is_set)
{
  int i, idx_in_cache;
  char *classname = NULL;
 ...
 *   attr_info(in):
 */
int heap_attrinfo_set_uninitialized_global(THREAD_ENTRY *thread_p, OID *inst_oid, RECDES *recdes,
                                           HEAP_CACHE_ATTRINFO *attr_info)
{
  if (attr_info == NULL)
  {
 ...
 */
int heap_get_class_info(THREAD_ENTRY *thread_p, const OID *class_oid, HFID *hfid_out,
 ...
 *
 */
int heap_get_btid_from_index_name(THREAD_ENTRY *thread_p, const OID *p_class_oid, const char *index_name,
                                  BTID *p_found_btid)
{
  int error = NO_ERROR;
  int classrepr_cacheindex = -1;
 ...
 */
int heap_object_upgrade_domain(THREAD_ENTRY *thread_p, HEAP_SCANCACHE *upd_scancache, HEAP_CACHE_ATTRINFO *attr_info,
                               OID *oid, const ATTR_ID att_id)
{
  ...
  for (i = 0, value = attr_info->values; i < attr_info->num_values; i++, value++)
  {
    ...
    if (QSTR_IS_ANY_CHAR_OR_BIT(src_type) && QSTR_IS_ANY_CHAR_OR_BIT(dest_type))
    {
      ...
      assert((QSTR_IS_ANY_CHAR(src_type) && QSTR_IS_ANY_CHAR(dest_type)) || (!QSTR_IS_ANY_CHAR(src_type) && !QSTR_IS_ANY_CHAR(dest_type)));

      assert((QSTR_IS_BIT(src_type) && QSTR_IS_BIT(dest_type)) || (!QSTR_IS_BIT(src_type) && !QSTR_IS_BIT(dest_type)));

      /* check string truncation */
      if (dest_prec < curr_prec)
      {
        if (prm_get_bool_value(PRM_ID_ALTER_TABLE_CHANGE_TYPE_STRICT) == true || prm_get_bool_value(PRM_ID_ALLOW_TRUNCATED_STRING) == false)
        {
    ...
    if (TP_IS_CHAR_TYPE(TP_DOMAIN_TYPE(dest_dom)) && !(TP_IS_CHAR_TYPE(src_type) || src_type == DB_TYPE_ENUMERATION) && prm_get_bool_value(PRM_ID_ALTER_TABLE_CHANGE_TYPE_STRICT) == false && prm_get_bool_value(PRM_ID_ALLOW_TRUNCATED_STRING) == true)
    {
    ...
    if (error != NO_ERROR)
    {
      ...
      if (prm_get_bool_value(PRM_ID_ALTER_TABLE_CHANGE_TYPE_STRICT) == true || (TP_IS_CHAR_TYPE(TP_DOMAIN_TYPE(dest_dom)) && prm_get_bool_value(PRM_ID_ALLOW_TRUNCATED_STRING) == false))
      {
        error = ER_ALTER_CHANGE_TRUNC_OVERFLOW_NOT_ALLOWED;
        er_set(ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 0);
        goto exit;
  ...
  if (updated_n_attrs_id != 1 || attr_info->read_classrepr == NULL || attr_info->last_classrepr == NULL || attr_info->read_classrepr->id >= attr_info->last_classrepr->id)
  {
    error = ER_UNEXPECTED;
 ...
 *   func_indx_preds(out):
 */
int heap_init_func_pred_unpack_info(THREAD_ENTRY *thread_p, HEAP_CACHE_ATTRINFO *attr_info, const OID *class_oid,
 ...
 */
void heap_free_func_pred_unpack_info(THREAD_ENTRY *thread_p, int n_indexes, FUNC_PRED_UNPACK_INFO *func_indx_preds,
 ...
 */
int heap_header_capacity_start_scan(THREAD_ENTRY *thread_p, int show_type, DB_VALUE **arg_values, int arg_cnt,
                                    void **ptr)
{
  int error = NO_ERROR;
 ...
 */
bool heap_attrinfo_check_unique_index(THREAD_ENTRY *thread_p, HEAP_CACHE_ATTRINFO *attr_info, ATTR_ID *att_id,
                                      int n_att_id)
{
  OR_INDEX *index;
  int num_btids, i, j, k;
 ...
 */
int heap_get_header_page(THREAD_ENTRY *thread_p, const HFID *hfid, VPID *header_vpid)
{
...
heap_insert_adjust_recdes_header(THREAD_ENTRY *thread_p, HEAP_OPERATION_CONTEXT *insert_context, bool is_mvcc_class)
{
  ...
  use_optimization = (is_mvcc_class && (insert_context->update_in_place == UPDATE_INPLACE_NONE) && (!(mvcc_flags & OR_MVCC_FLAG_VALID_PREV_VERSION)) && !heap_is_big_length(record_size + OR_MVCCID_SIZE) && !insert_context->is_bulk_op);
#endif
  ...
  if (use_optimization)
  {
     ...
     */
    // 가장 일반적인 경우. UPDATE_INPLACE_NONE 상태이므로, 헤더에 DELID가 포함되지 않는다. 헤더 조정을 최적화한다.
    ...
    //mvccid 의 pos 포인터 주소는 data의 시작주소로부터 OR_MVCC_INSERT_ID_OFFSET(8)만큼 떨어져있음
    ...
    /* Sets the MVCC INSID */
    // id값을 를 new_ins_mvccid_pos_p 주소에 넣음(int64)
...
heap_update_adjust_recdes_header(THREAD_ENTRY *thread_p, HEAP_OPERATION_CONTEXT *update_context, bool is_mvcc_class)
{
  ...
  if (use_optimization)
  {
    ...
    if ((mvcc_flags & update_mvcc_flags) != update_mvcc_flags)
    {
      ...
      assert(update_context->recdes_p->area_size >= update_context->recdes_p->length + CAST_BUFLEN(new_data_p - existing_data_p));
                                   ...
                                   PGBUF_WATCHER *home_hint_p)
{
  ...
  // 전체 슬롯 수만큼 반복하며 빈 슬롯을 찾고, 빈 슬롯을 찾는다면 context->res_oid.slotid에 저장
...
heap_get_record_location(THREAD_ENTRY *thread_p, HEAP_OPERATION_CONTEXT *context)
{
  ...
  if (context->scan_cache_p != NULL && context->scan_cache_p->page_watcher.pgptr != NULL && context->scan_cache_p->cache_last_fix_page == true)
  {
  ...
  if (context->scan_cache_p != NULL)
  {
    ...
    assert(heap_get_class_oid_from_page(thread_p, context->home_page_watcher_p->pgptr, &heap_class_oid) ==
           NO_ERROR);
    assert((OID_EQ(&heap_class_oid, &context->scan_cache_p->node.class_oid)) || (OID_ISNULL(&context->scan_cache_p->node.class_oid) && spage_get_record_type(context->home_page_watcher_p->pgptr,
...
heap_update_relocation(THREAD_ENTRY *thread_p, HEAP_OPERATION_CONTEXT *context, bool is_mvcc_op)
{
  ...
  if (remove_old_forward)
  {
    ...
    if ((new_home_recdes.type == REC_RELOCATION || new_home_recdes.type == REC_BIGONE) && context->forward_page_watcher_p->page_was_unfixed)
    {
      /*
...
heap_update_home(THREAD_ENTRY *thread_p, HEAP_OPERATION_CONTEXT *context, bool is_mvcc_op)
{
  ...
  if ((home_page_updated_recdes_p->type == REC_RELOCATION || home_page_updated_recdes_p->type == REC_BIGONE) && context->home_page_watcher_p->page_was_unfixed)
  {
    /*
     * Need to get the record again, since record may have changed
     * by other transactions (INSID removed by VACUUM, page compact).
 ...
 */
void heap_create_insert_context(HEAP_OPERATION_CONTEXT *context, HFID *hfid_p, OID *class_oid_p, RECDES *recdes_p,
 ...
 *   scancache_p(in): scan cache to use (optional)
 */
void heap_create_delete_context(HEAP_OPERATION_CONTEXT *context, HFID *hfid_p, OID *oid_p, OID *class_oid_p,
                                HEAP_SCANCACHE *scancache_p)
{
  assert(context != NULL);
  assert(hfid_p != NULL);
 ...
 */
void heap_create_update_context(HEAP_OPERATION_CONTEXT *context, HFID *hfid_p, OID *oid_p, OID *class_oid_p,
...
/* 힙에 객체 삽입
1. 객체가 단일 페이지에 삽입될 수 없는 경우, 다중 페이지 객체로 오버플로우 영역에 삽입
이때, 오버 플로우 주소(실제 객체 내용에 대한 주소 매핑)을 나타내는 오버플로우 재배치 레코드가 힙 내에 생성
2. 객체가 페이지에 예약된 공간을 초과하지 않고 마지막으로 할당된 페이지에 삽입될 수 있다면, 
해당 페이지에 객체를 배치
3. 객체가 페이지에 예약된 공간을 초과하지 않고, 힌트된 페이지에 삽입될 수 있다면 해당 페이지에 객체 배치
4. 객체는 새로 할당된 페이지에 삽입. 이 경우 예약 공간은 고려 안함*/
int heap_insert_logical(THREAD_ENTRY *thread_p, HEAP_OPERATION_CONTEXT *context, PGBUF_WATCHER *home_hint_p)
{
   ...
   */
  if (!OID_ISNULL(&context->class_oid) && !OID_IS_ROOTOID(&context->class_oid) && context->recdes_p->type != REC_ASSIGN_ADDRESS)
  {
    // insert operation 을 위해 record header 를 조정함
    // mvcc id를 data 영역에 저장함(start_p + OR_MVCC_INSERT_ID_OFFSET;)
   ...
   */
  if (context->scan_cache_p != NULL && context->scan_cache_p->cache_last_fix_page == true && (context->home_page_watcher_p == &context->home_page_watcher || context->home_page_watcher_p == home_hint_p))
  {
    /* cache */
    assert(context->home_page_watcher_p->pgptr != NULL);
  ...
  if (context->do_supplemental_log && !LSA_ISNULL(&context->supp_redo_lsa) && context->recdes_p->type != REC_ASSIGN_ADDRESS)
  {
    (void)log_append_supplemental_lsa(thread_p,
                                      thread_p->trigger_involved ? LOG_SUPPLEMENT_TRIGGER_INSERT : LOG_SUPPLEMENT_INSERT, &context->class_oid, NULL, &context->supp_redo_lsa);
  }
...
int heap_delete_logical(THREAD_ENTRY *thread_p, HEAP_OPERATION_CONTEXT *context)
{
  ...
  if (spage_get_record(thread_p, context->home_page_watcher_p->pgptr, context->oid.slotid, &context->home_recdes, COPY) != S_SUCCESS)
  {
    rc = ER_FAILED;
    goto error;
  }
  ...
  {
    (void)log_append_supplemental_lsa(thread_p,
                                      thread_p->trigger_involved ? LOG_SUPPLEMENT_TRIGGER_DELETE : LOG_SUPPLEMENT_DELETE, &context->class_oid, &context->supp_undo_lsa, NULL);
  }

error:

  /* unfix or keep home page */
  if (context->scan_cache_p != NULL && context->home_page_watcher_p == &context->home_page_watcher && context->scan_cache_p->cache_last_fix_page == true)
  {
    pgbuf_replace_watcher(thread_p, context->home_page_watcher_p, &context->scan_cache_p->page_watcher);
  }
  else
...
heap_update_logical(THREAD_ENTRY *thread_p, HEAP_OPERATION_CONTEXT *context)
{
...
#if defined(SERVER_MODE)
  assert((!is_mvcc_op && HEAP_IS_UPDATE_INPLACE(context->update_in_place)) || (is_mvcc_op && !HEAP_IS_UPDATE_INPLACE(context->update_in_place)));
  ...
  {
    (void)log_append_supplemental_lsa(thread_p,
                                      thread_p->trigger_involved ? LOG_SUPPLEMENT_TRIGGER_UPDATE : LOG_SUPPLEMENT_UPDATE, &context->class_oid, &context->supp_undo_lsa,
...
int heap_vacuum_all_objects(THREAD_ENTRY *thread_p, HEAP_SCANCACHE *upd_scancache, MVCCID threshold_mvccid)
{
  ...
  while (!VPID_ISNULL(&next_vpid))
  {
    ...
    if (worker.n_heap_objects > 0 && heap_page_get_vacuum_status(thread_p, pg_watcher.pgptr) != HEAP_PAGE_VACUUM_NONE)
    {
 ...
 */
int heap_cache_class_info(THREAD_ENTRY *thread_p, const OID *class_oid, HFID *hfid, FILE_TYPE ftype,
                    ...
                    char **classname_out)
{
  ...
  assert(entry->hfid.hpgid != NULL_PAGEID && entry->hfid.vfid.fileid != NULL_FILEID && entry->hfid.vfid.volid != NULL_VOLID && entry->classname != NULL);
 ...
 */
int heap_get_hfid_if_cached(THREAD_ENTRY *thread_p, const OID *class_oid, HFID *hfid_out, FILE_TYPE *ftype_out,
...
{
  if (current_freespace > prev_freespace && current_freespace > HEAP_DROP_FREE_SPACE && prev_freespace < HEAP_DROP_FREE_SPACE)
  {
    return true;
  }
  return false;
...
heap_get_visible_version_internal(THREAD_ENTRY *thread_p, HEAP_GET_CONTEXT *context, bool is_heap_scan)
{
  ...
  if (context->scan_cache != NULL && context->scan_cache->mvcc_snapshot != NULL && context->scan_cache->mvcc_snapshot->snapshot_fnc != NULL && !mvcc_is_mvcc_disabled_class(context->class_oid_p))
  {
 ...
 */
int heap_prepare_object_page(THREAD_ENTRY *thread_p, const OID *oid, PGBUF_WATCHER *page_watcher_p,
 ...
 */
void heap_init_get_context(THREAD_ENTRY *thread_p, HEAP_GET_CONTEXT *context, const OID *oid, OID *class_oid,
...
void heap_scancache::reserve_area(size_t size)
{
  alloc_area();
  m_area->reserve(size);
}

void heap_scancache::assign_recdes_to_area(RECDES &recdes, size_t size /* = 0 */)
{
  reserve_area(size);
...
int heap_alloc_new_page(THREAD_ENTRY *thread_p, HFID *hfid, OID class_oid, PGBUF_WATCHER *home_hint_p,
                        VPID *new_page_vpid)
{
  int error_code = NO_ERROR;
  HEAP_CHAIN new_page_chain;
...
void heap_log_postpone_heap_append_pages(THREAD_ENTRY *thread_p, const HFID *hfid, const OID *class_oid,
                                         const std::vector<VPID> &heap_pages_array)
{
  ...
  int log_data_size = (DB_ALIGN(OR_HFID_SIZE, PTR_ALIGNMENT) + OR_OID_SIZE + sizeof(int) + array_size * DISK_VPID_ALIGNED_SIZE);
