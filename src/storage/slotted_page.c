...
spage_verify_header(PAGE_PTR page_p)
{
  ...
  if (sphdr == NULL || sphdr->total_free < 0 || sphdr->cont_free < 0 || sphdr->cont_free > sphdr->total_free || sphdr->offset_to_free_area >= SPAGE_DB_PAGESIZE || (PTR_ALIGN(page_p + sphdr->offset_to_free_area, sphdr->alignment) != page_p + sphdr->offset_to_free_area) || sphdr->num_records < 0 || sphdr->num_slots < 0 || sphdr->num_records > sphdr->num_slots)
  {
...
bool spage_is_valid_anchor_type(const INT16 anchor_type)
{
  ...
  return (anchor_type == ANCHORED || anchor_type == ANCHORED_DONT_REUSE_SLOTS || anchor_type == UNANCHORED_ANY_SEQUENCE || anchor_type == UNANCHORED_KEEP_SEQUENCE);
}
...
int spage_max_space_for_new_record(THREAD_ENTRY *thread_p, PAGE_PTR page_p)
{
  ...
  if ((page_header_p->anchor_type != ANCHORED_DONT_REUSE_SLOTS) && (page_header_p->num_slots > page_header_p->num_records))
  {
    if (total_free < 0)
    {
      total_free = 0;
...
void spage_initialize(THREAD_ENTRY *thread_p, PAGE_PTR page_p, INT16 slot_type, unsigned short alignment, bool is_saving)
{
  ...
  assert(alignment == CHAR_ALIGNMENT || alignment == SHORT_ALIGNMENT || alignment == INT_ALIGNMENT || alignment == LONG_ALIGNMENT || alignment == FLOAT_ALIGNMENT || alignment == DOUBLE_ALIGNMENT);
...
int spage_update(THREAD_ENTRY *thread_p, PAGE_PTR page_p, PGSLOTID slot_id, const RECDES *record_descriptor_p)
{
  ...
  if (page_header_p->is_saving && spage_save_space(thread_p, page_header_p, page_p, page_header_p->total_free - total_free_save) != NO_ERROR)
  {
    assert(false);
 ...
 *   mvcc_insert_record_length(in): the length of insert record
 */
bool spage_is_mvcc_updatable(THREAD_ENTRY *thread_p, PAGE_PTR page_p, PGSLOTID slot_id, int delete_record_length,
...
bool spage_reclaim(THREAD_ENTRY *thread_p, PAGE_PTR page_p)
{
  ...
  if (page_header_p->num_slots > 0)
  {
    ...
    for (slot_id = page_header_p->num_slots - 1; slot_id >= 0; slot_id--)
    {
      ...
      if (slot_p->offset_to_record == SPAGE_EMPTY_OFFSET && (slot_p->record_type == REC_MARKDELETED || slot_p->record_type == REC_DELETED_WILL_REUSE))
      {
...
int spage_split(THREAD_ENTRY *thread_p, PAGE_PTR page_p, PGSLOTID slot_id, int offset, PGSLOTID *out_new_slot_id_p)
{
  ...
  else
  {
    ...
    if (page_header_p->is_saving && spage_save_space(thread_p, page_header_p, page_p,
...
int spage_take_out(THREAD_ENTRY *thread_p, PAGE_PTR page_p, PGSLOTID slot_id, int takeout_offset, int takeout_length)
{
  ...
  else
  {
    ...
    if (((int)slot_p->record_length - takeout_offset - takeout_length) > 0)
    {
      ...
      if (SPAGE_OVERFLOW(slot_p->offset_to_record + takeout_offset + (slot_p->record_length - takeout_offset - takeout_length)))
      {
        er_set(ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
        assert_release(false);
        return SP_ERROR;
                 ...
                 const RECDES *record_descriptor_p, bool is_append)
{
  ...
  if (spage_is_record_located_at_end(page_header_p, slot_p) && space <= page_header_p->cont_free)
  {
    ...
    if (!is_append)
    {
      ...
      if (SPAGE_OVERFLOW(slot_p->offset_to_record + offset + record_descriptor_p->length + (slot_p->record_length - offset)))
      {
        er_set(ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
        assert_release(false);
        return SP_ERROR;
  ...
  else if ((int)record_descriptor_p->length + (int)slot_p->record_length <= page_header_p->cont_free)
  {
    ...
    else
    {
      ...
      memcpy((char *)page_p + page_header_p->offset_to_free_area, (char *)page_p + slot_p->offset_to_record,
             offset);
      if (SPAGE_OVERFLOW(page_header_p->offset_to_free_area + offset + record_descriptor_p->length + (slot_p->record_length - offset)))
      {
        er_set(ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
        assert_release(false);
        return (SP_ERROR);
 ...
 *       append.
 */
int spage_overwrite(THREAD_ENTRY *thread_p, PAGE_PTR page_p, PGSLOTID slot_id, int overwrite_offset,
...
int spage_merge(THREAD_ENTRY *thread_p, PAGE_PTR page_p, PGSLOTID first_slot_id, PGSLOTID second_slot_id)
{
  ...
  if (spage_is_record_located_at_end(page_header_p, first_slot_p) && (int)second_slot_p->record_length <= page_header_p->cont_free)
  {
    /*
  ...
  else
  {
    ...
    page_header_p->total_free += (first_slot_p->record_length + second_slot_p->record_length + first_old_waste + second_old_waste);
    ...
    /* Move the old data to the end */
    if (SPAGE_OVERFLOW(page_header_p->offset_to_free_area + first_slot_p->record_length + second_slot_p->record_length))
    {
      er_set(ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
      assert_release(false);
      return SP_ERROR;
...
int spage_mark_deleted_slot_as_reusable(THREAD_ENTRY *thread_p, PAGE_PTR page_p, PGSLOTID slot_id)
{
  ...
  if (slot_p && slot_p->offset_to_record == SPAGE_EMPTY_OFFSET && (slot_p->record_type == REC_MARKDELETED || slot_p->record_type == REC_DELETED_WILL_REUSE))
  {
    ; /* OK */
  }
  else
...
int spage_check(THREAD_ENTRY *thread_p, PAGE_PTR page_p)
{
       ...
       SSIZEOF(SPAGE_SLOT) * page_header_p->num_slots) > SPAGE_DB_PAGESIZE)
  {
    snprintf(err_msg, sizeof(err_msg),
             ...
             (page_header_p->cont_free + page_header_p->offset_to_free_area + sizeof(SPAGE_SLOT) * page_header_p->num_slots), SPAGE_DB_PAGESIZE);
...
spage_is_unknown_slot(PGSLOTID slot_id, SPAGE_HEADER *page_header_p, SPAGE_SLOT *slot_p)
{
  ...
  return (slot_id < 0 || slot_id >= page_header_p->num_slots || slot_p->offset_to_record == SPAGE_EMPTY_OFFSET || slot_p->offset_to_record < sizeof(SPAGE_HEADER) || slot_p->offset_to_record > max_offset);
}
